...
`b01688b8-2d0c-4d0c-a546-464f154f7de9`, and degraded map
`94a32c78-6adc-4af0-ae39-f60aa658331b`. Frozen `field-aware-v3` recall improved only to
**20/30 (66.7%)**, below 27/30. The map kept 179 responsibilities and dropped 11, used all five
omission retries, moved omissions from 92 to 84, and accepted grounded quote repair that reduced
root quote failures from 10 to 6. Reader use was 25/40 calls, 62,680/500,000 input tokens, one
general repair, and `$0.3134/$10` estimated input cost. No responsibility call truncated.
Merge/apply stayed false, post-pass limits stayed 1/5/1, and business objects, versions, and changes
stayed zero. The outer document was still in the later claim phase; the R2 map/job was terminal.

Grok 4.5 reviewed the failure and said **continue R2 with Batch F**, but only as a bounded
span-anchored field-fidelity change. Do not do another free-form prompt/retry batch. Batch F must
force one selected duty span to one validated RAO, check action direction and object fidelity
against that span, prefer short structured duties, split multi-verb chains, and keep focused
context limited to the selected spans. Frozen scorer, fixture, quote policy, budgets, flags, and
zero-write rules remain unchanged. After one reviewed Batch F release and one fresh gate:
`>=27/30` proceeds to later R2 shadow-merge work; `24–26/30` stops code churn and permits a bounded
model bake-off only on the span-anchored path; `<=23/30` stops completeness batches and requires an
owner decision between bake-off and deeper architecture. Full evidence is in
...
| Migration 65 generated/fresh-DB drift warning, resolved 2026-07-29 | [Reliability plan REL-5](plan_repo_reliability_and_release_gaps.md#rel-5-migration-65-and-snapshot-drift) |
...
- Batch E shipped and live-tested. Duty-complete base reads, one durable synthetic read, five
...
### Audit of `fix_enhancement.md` implementation + uncommitted-source risk (2026-07-04)

What changed:
This session audited how well `fix_enhancement.md` was implemented (no code was changed). Finding: the macro/holistic tier is far more complete than the doc's §6.9 records. Verified present in code: dedicated `macro` route + `model_pool_macro` fallback (primary `openai/gpt-4.1-mini`, NOT Gemini — Gemini rejects the nested schema), `job_runs` + `documents.macro_health` observability with the Admin → Documents badge, coverage-first (round-major) lens budget, Bug C/E fixes, **Bug G fixed** (coverage now chained after macro inside `macro-relationship-extraction.ts`, not raced), **Bug D fixed** (`statusForGeneratedMacroRelationship` now births generated relationships `pending_review`, admin-visible, while read-time serving still requires approved support), and the **`source_workflow_maps` artifact** with edge-level dedup (`candidate-hash.ts` `stableGraphKey`) and deterministic macro derivation from graph paths.

Why this matters:
- **The source is UNCOMMITTED.** All of the above is in the working tree only (`schema.ts`, `lifecycle.ts`, `candidate-hash.ts`, the three macro workers, `macro-relationship-extraction.ts`, untracked `packages/db/migrations/sql/86_source_workflow_maps.sql`, `apps/workers/src/lib/schema-repair.ts`, `packages/ai/src/__verify__/workflow-map-fixture.ts`, `packages/db/src/verify-macro-support-queries.ts`, `scripts/{export-worker-failures,verify-workflow-map-prod}.mjs`). Prod workers/migrations ARE deployed per the block above, so **the deployed prod state is ahead of `main` — commit the working tree before any further worker deploy or you risk shipping from a tree that no longer matches prod.**
- **`fix_enhancement.md` §6.9 is STALE and actively misleading** — it lists Bug G, the admin `macro_health` UI, and the workflow-map artifact as "still not done" when they are done. A verification subagent this session trusted §6.9's framing and wrongly reported those items absent. Future sessions: **verify macro state against the working tree, not against §6.9.** A correction banner was added to §6.9.
...
- Hand-written idempotent migration `packages/db/migrations/sql/65_document_context_and_domain_hints.sql` adds nullable `documents.context` (text) and `documents.domain_hints` (jsonb), applied to prod **and** added to `schema.ts`. This historical warning is resolved as of 2026-07-29: the full migration runner applies `sql/65` on fresh databases, the latest Drizzle snapshot records the same shape, a focused CI verifier checks the applied columns, and the generated-migration journal has no drift. Keep raw migration 65 as the owner; do not add duplicate generated DDL.
