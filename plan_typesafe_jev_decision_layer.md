...
Publication gate: **not approved**. Exact-head review `20260922T213853-14212-27125` rejected commit `f0e0975`; this documentation-only draft is repairing its four findings and must be committed, pushed, and independently re-reviewed before a PR is opened. No implementation step is authorized from this draft.
...
- Make deployed database-role separation a Step 1A prerequisite, because today's Vercel routes, Trigger workers, and migrations share `DATABASE_URL`/`DIRECT_URL` paths. Without printing credentials, query and record `current_user`, table/function owners, `rolbypassrls`, and effective privileges through the actual Vercel Production, Trigger `prod`, migration, and Supabase `service_role` connections. Provision distinct `oracle_web_runtime` and `oracle_worker_runtime` login roles with `NOSUPERUSER NOBYPASSRLS NOCREATEDB NOCREATEROLE`; create `oracle_jev_owner NOLOGIN` to own the Jev tables, reserved-settings trigger, and fixed-`search_path` `SECURITY DEFINER` functions. Keep the migration-owner connection only in the guarded migration/administration path and absent from Vercel and Trigger. Vercel `DATABASE_URL` uses the web role; add `WORKER_DATABASE_URL` for Trigger's unpooled worker role; retain `DIRECT_URL` only for guarded migrations and refactor every deployed `getDirectDb()` caller to an explicitly least-privilege runtime client before Step 1B ships.
- Enable row-level security on all nine new `oracle_decision_*` tables in the foundation migration. Deny anonymous access; authenticated non-admin users can neither select nor mutate; authenticated admins may select bounded audit/control data but browser roles never mutate. Revoke direct `INSERT/UPDATE/DELETE` from `PUBLIC`, `anon`, `authenticated`, `service_role`, both runtime roles, and generic application roles. Grant the two runtime roles only the exact reads and typed function `EXECUTE` privileges their call paths need. Those named functions are owned by `oracle_jev_owner`, use fixed `search_path`, validate their server/admin caller, and take the required row locks; the reserved-settings trigger permits mutation only when `current_user=oracle_jev_owner` inside those functions. A direct web/worker/admin/`service_role` statement never qualifies by session metadata. Keep the admin check in RLS and the function, so API, RPC, or direct database access cannot bypass it.
- Encode database mutation allowlists rather than relying on “immutable” prose. Run identity/config columns and the complete subject set freeze before transport and never change. A pending run may transition once to a terminal status while only its enumerated answer/usage/error/comparator/review/finalization columns fill; terminal outcomes cannot change except the guarded review-head fields. Data-policy version/class/evidence fields never change; only its named revocation function may set status/revoked time. Spend epoch identity/cap/approval never change; named reserve, settle, and close functions may update only reserved/spent/status/closed time. Adjudications and control events are insert-only; canary reservations are immutable; counters change only through the reservation functions. Separate typed functions own pending-run+subject creation, finalization/effect, source erasure, policy revocation, epoch reserve/settle/close, adjudication/head advance, and control transition, each atomically writing its required event where applicable.
...
- `.env.example`, `turbo.json`, `docs/configuration.md`, and `docs/deployment.md`: document `TYPESAFE_API_KEY`, pinned model override for non-production evals only, per-use-case feature flags, the 1Password location, the separated `DATABASE_URL`/`WORKER_DATABASE_URL`/migration-only `DIRECT_URL` contract, and the independent secret-delivery/redeploy gates. Vercel Production uses its authenticated environment-variable flow; Trigger `prod` uses the documented management API and PAT, then a worker redeploy. Keep secrets server-side and verify names/presence only.
...
- Split foundation delivery into two ordered PRs/outcomes. **Step 1A (schema and role boundary only):** merge the reviewed migration/schema types and role/grant topology with no web or worker reader, run the guarded production migration from that merged SHA, verify the journal, real connection identities, table/function ownership, grants, and all nine RLS policies, then smoke unchanged runtime behavior. **Step 1B (dependent code and credential cutover):** only after Step 1A proof, merge/deploy the separated runtime clients, decision client, controls, pages, banner, and workers; cut Vercel/Trigger to their least-privilege URLs, prove the migration credential is absent, and smoke the admin layout and decision page again. Because Vercel deploys automatically from `main`, no commit that imports or queries a new decision table may merge before the production migration is verified.
...
- Direct-access tests exercise anonymous, authenticated non-admin/admin, Supabase `service_role`, `oracle_web_runtime`, and `oracle_worker_runtime` against every new table: browser reads follow the bounded policy, every direct mutation fails, and only granted typed functions mutate and emit exactly one control event. Fresh-install tests create the real owner/runtime topology. Post-deploy read-only probes through the actual Vercel and Trigger credentials prove distinct expected `current_user` values, neither is a table/function owner or `BYPASSRLS`, direct mutation privileges are absent, and only intended function execution privileges exist. The migration credential is absent from both deployed environments.
...
- Seed those settings in the foundation migration. Reserve every `jev_*` setting key at the database and generic API boundaries: generic GET may return authorized bounded controls, but `apps/web/app/api/admin/settings/route.ts` rejects every generic mutation of that namespace. A `BEFORE INSERT/UPDATE/DELETE` trigger on `settings` rejects reserved-key mutation unless `current_user=oracle_jev_owner` inside the named fixed-`search_path` `SECURITY DEFINER` transition function; authenticated-admin, web-runtime, worker-runtime, migration-owner runtime misuse, and direct `session_user=service_role` SQL remain rejected. The function accepts a typed use-case/action rather than an arbitrary key/value. Add an admin-only “Jev decision controls” section under `apps/web/app/admin/settings/page.tsx` with a focused server action/component that calls only that function: `off→shadow` requires pinned requested-model/pack/threshold/active-data-policy versions; completed shadow evaluation records the exact provider-returned resolved model in the evidence artifact. For behavioral pilots, `shadow→active` requires that durable approved resolved model, committed held-out artifact URL, and independent-review commit bound to the exact fingerprint; the reconciled-answer audit rejects this transition entirely. Any requested/approved model, pack, threshold, or policy change is one locked transition that forces behavioral `active→shadow`, clears evidence, increments generation, and writes an event. Every active response matches requested/approved resolved models or is discarded with atomic demotion. Every transition/fallback/off/revocation uses the same lock/generation/event path; no environment/direct-settings/dashboard bypass exists.
...
- Migration/schema verification proves the dedicated tables, constraints, indexes, generated types, owner/runtime role topology, reserved-settings trigger, same-run composite adjudication FKs, column/state allowlist triggers, and function/grant boundaries match this contract. For every append-only table, direct web-runtime, worker-runtime, authenticated-admin, and `service_role` update/delete fail; for every immutable identity/config column and illegal state transition, direct mutation fails. Direct generic-API and SQL insert/update/delete of any `jev_*` setting fail, while each permitted typed transition succeeds only through its named database function and records its event. Cross-run `supersedes_id`, cross-run current-head assignment, and nonzero-version/null-head combinations fail at the database constraint.
- Retention/erasure tests delete fixture messages, document chunks, claim/candidate evidence, claims, documents, supplied entities, Brain sections, relationships, model runs, and reviewer employees; typed links cascade/null as specified, no stale identifier/free-text survives, and unrelated evidence remains. Deleting one extraction source or replacing exact evidence fires the erasure trigger, removes surviving parent/batch subjects despite compatibility JSON/parents, dismisses review, and makes the run pruneable. An aged run referenced by an unacknowledged event deletes while the event survives with null FKs/bounded snapshot.
...
- Configuration tests enumerate every named mode/model/pack/threshold/data-policy/evidence key, prove migration defaults `off` with only `v1-public-synthetic` active, reject missing evidence/version/policy metadata and pack-policy widening, deny direct transitions through the generic settings API and web-runtime/worker-runtime/authenticated-admin/`service_role` SQL, and prove generic GET remains read-only. Typed functions succeed only for expressly granted callers and persist one control event per compare-and-set; policy revocation blocks calls/demotes dependent modes, and automatic fallback uses the same guarded function without touching another key. Document-classification tests prove every runtime/admin role's direct **insert and update** reject `public_synthetic` alone or mixed, while the action exposes only the six non-synthetic classes. For each fingerprint component, change it while active and prove the transaction forces shadow, clears old evidence, increments generation, discards an in-flight/late result, and refuses reactivation until new held-out artifact plus review commit match the exact new fingerprint. Simulate resolved-model drift and prove the same fail-safe demotion.
...
- The current entity selector does not persist `model_runs`. Represent this use case with `oracle_decision_subjects.entity_query_key`, a hash of normalized query + registry version, a query-origin subject tied to **every** chat message consumed by entity planning (currently up to three user turns), every worker source message/chunk, or the short-lived MCP request that produced it, **and one typed `entity_id` subject for every registry candidate actually serialized to Jev**, not only selected entities; absence or an incomplete contributing-message/candidate set blocks the call. The decision/input keys hash the ordered contributing message IDs/content hashes, candidate IDs, entity types, and hashes of canonical value/display label/aliases, plus locale and pack/threshold/data-policy/requested-model versions. Mutation changes the hash and prevents reuse; deletion of any contributing message or supplied entity triggers whole-run erasure. Store the current selector's bounded chosen IDs, route/model ID, attempt status, latency, and usage returned by `OracleAIClient` in the decision run's comparator fields; never claim a nonexistent model-run link.
...
- A three-turn follow-up links all three contributors; a topic change links only the current message; no message links twice. Missing/changed contributors block transport/reuse, and deleting each linked turn independently discards the run. Candidate/message subjects commit before mocked transport begins.
...
- In `packages/db/src/schema.ts`, the generated migration, and a listed hand-written SQL migration, add `messages.extraction_directive` (`normal|force_extract`, default `normal`), nullable `capture_request_source` (`command|composer_action`), `capture_requested_at`, and `capture_requested_by_employee_id`, plus the new batch statuses/snapshot fields and normalized authoritative window membership. A database constraint requires every capture field null for `normal`; `force_extract` requires a user-role message, non-null message employee, source/time/requester, and requester equal to that employee. Update `messages_self_insert`/`messages_self_update` so authenticated browser SQL may create/update only `normal` rows and cannot set or alter capture audit fields. Merge this backward-compatible schema alone; run `pnpm db:migrate`, prove journal/constraints/RLS/indexes/backfill on production, and smoke unchanged workers. No worker/web code may import or query the new schema before that proof.
- Add `extraction_batch_messages(extraction_batch_id uuid FK ON DELETE CASCADE, message_id uuid FK ON DELETE CASCADE, position integer NOT NULL CHECK (position>=0), PRIMARY KEY(extraction_batch_id,message_id), UNIQUE(extraction_batch_id,position))` plus index `(message_id, extraction_batch_id)`. Backfill it from ordered `source_message_ids`. A compatibility trigger validates legacy JSON UUID arrays and mirrors inserts/updates into the relation until every writer migrates; JSON remains compatibility/audit data, never reconciliation authority.
...
- In `apps/web/app/api/messages/route.ts`, change the strict request body to `{channelId,content,intent?:'message'|'capture'}` and never accept directive, actor, source, requester, or timestamp fields. After `requireEmployee`, channel-participant authorization, and active-channel validation, derive capture server-side only when `intent==='capture'` or content begins with the exact anchored `/capture ` command followed by non-empty text; strip only that prefix before storage. The employee becomes both message owner and capture requester. Capture cannot target an existing message, another employee, or another channel; near matches, quoted commands, arbitrary metadata, imports, Teams, assistants, and ordinary messages remain `normal`.
...
- Include the durable directive in input identity. Any window containing `force_extract` bypasses Jev before transport in shadow/active modes and reaches the unchanged extractor; edits re-run the deterministic parser and invalidate old decisions. No natural-language phrase such as “remember this” grants the bypass.
...
- Link each decision to its owner and every source message. Atomically finalize the decision, subjects, skip status, and reconciled messages. The key hashes ordered message IDs/content/directives; changed input cannot reuse a skip.
...
- Typecheck and the new verifier pass in both dispatch modes. Handler tests cover unauthenticated/nonparticipant/inactive-channel requests, malformed or extra privileged fields, empty/exact `/capture`, composer intent, and ordinary messages. Database tests prove constraints and authenticated direct insert/update cannot set or alter capture fields. UI tests cover toggle request/reset/restore, command path, badge, Realtime, and initial-render parity. Imports, Teams/assistant messages, quoting, and near matches remain normal. Exact command/action windows make zero Jev suppression calls and always reach current extraction through overlap/edit/retry/replay.
- Cross-run fixtures create owners in two/three job runs: late old-run completion beats skip/failure; replay racing a new extractor serializes; concurrent sync/batch order converges. Backfill/compatibility rows equal JSON; duplicate membership/position, bad FK/UUID, and orphan writes fail; `EXPLAIN` uses `(message_id, extraction_batch_id)`.
...
- For `insert_new_claim`, allow preserves normal insertion; review creates only `pending_review`, sets the source candidate `review_held`, and permanently vetoes document auto-approval. For `append_to_existing_claim`, review leaves the existing claim, approval, evidence, entities, and domains unchanged and sets the candidate `review_held` for admin review. Fallback preserves current promotion, except an inserted document claim remains pending review rather than auto-approved.
...
- In the same transaction that applies the disposition, add the resulting/new or existing claim as a subject and record effect `insert|duplicate_append|review_hold`. All existing schema, taxonomy, evidence, sensitivity-positive, and promotion rules remain authoritative; Jev may only add review/veto.
...
- Low-confidence/provider failure follows the explicit fallback above for both insertion and duplicate branches.
...
- The new verifier proves supported new/duplicate candidates insert/append once; every high-risk review case performs no unsafe duplicate mutation; evidence/hash/config drift between judgment and lock discards to fallback; concurrent duplicate workers cannot promote before the fence or append twice; and prompt-injected source data changes no deterministic rule. Admin-action tests deny non-admin/direct mutation; accepted held duplicates append exact validated evidence once without changing claim approval; rejection appends nothing; repeated submissions are idempotent; concurrent accept/accept and accept/reject serialize to one auditable winner; source/quote/target/decision/config drift and sensitivity/taxonomy failure remain held with zero partial writes. The UI exposes the count, bounded reason, target claim, evidence, and both actions.
...
- Add a check rejecting self-pairs and a database-authoritative unique expression index on `(LEAST(claim_a_id,claim_b_id),GREATEST(claim_a_id,claim_b_id))`. Preflight production for reversed/exact duplicates before migration. If any exist, a separately reviewed repair must repoint every `gaps`, `oracle_interventions`, and macro-change reference to one retained row before deleting duplicates; never silently discard linked artifacts.
...
- **Step 9B-1 is schema-only:** add nullable `model_runs.output_hash`/Drizzle `outputHash` through a journaled migration, merge without dependent code, migrate/verify production, and smoke chat/admin. Structured reconciliation runs hash canonical validated JSON; attachment generation hashes exact draft bytes. Step 9B-2 separately hashes the exact employee-visible rendered bytes in its decision identity. No dependent writer merges before production proof.
- **Step 9B-2 is shadow-only evaluation of current behavior on `499d8b0`:** ordinary no-attachment business answers already classify every supplied claim in a reconciliation ledger, render canonical summaries server-side, receive different-provider semantic review, allow exactly one ledger repair, and publish nothing after a second failure. After that existing review passes but before persistence, Jev evaluates bounded support/completeness/language against the exact validated ledger and employee-visible rendered answer. Its result records comparison only; it cannot authorize, suppress, rewrite, repair, retry, or fall back to the old interview-generated path.
- Create one query origin and link all contributing user messages, every supplied claim/evidence/source/relationship, the accepted reconciliation model run, independent review run, and exact ledger/rendered hashes. Identity includes locale, permission scope, configuration, canonical validated JSON hash, and rendered-byte hash. Deleting any source invalidates the shadow audit.
...
- Acceptance proves a no-attachment request follows reconciliation→independent review→optional Jev shadow and emits at most one employee answer; current repair remains exactly one; every included claim is assessed once; exceptions/ownership/handoffs/relationship premises survive the admission boundary; Chinese rejects untranslated claims and never exposes English relationship summaries; audit topology uses real reconciliation/review runs rather than invented draft/retry runs.
...
| Release ordering | Auto-deployed code queries new tables or retains migration-owner credentials | Step 1A/1B proves schema/RLS/roles first, then cuts Vercel/Trigger to distinct least-privilege URLs before dependent behavior; pre/post identity and smoke probes pass |
...
- Reconciliation audit schema: Step 9B-1 migration tests prove nullable `model_runs.output_hash` on fresh/current-snapshot databases; structured runs hash canonical validated JSON, attachment generation hashes exact bytes, and Jev decision identity hashes exact rendered employee-visible bytes.
