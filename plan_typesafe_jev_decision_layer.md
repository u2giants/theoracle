...
Publication gate: **not approved**. Exact-head review `20260922T224913-1365-18730` rejected commit `66205dc`; this documentation-only draft is repairing its seven high- and four medium-priority findings and must be committed, pushed, and independently re-reviewed before a PR is opened. No implementation step is authorized from this draft.
...
- Step 1B may land the explicit clients and tests unused. Step 1C first provisions `WORKER_DATABASE_URL` with the worker login while deployed workers still use `DIRECT_URL`, then refactors every worker caller, deploys/proves its identity and full manifest regression, and only then removes Trigger `DIRECT_URL`. Step 1D separately refactors every web caller to `getWebDb`, proves required `DATABASE_URL` with no fallback, switches it to the web login, redeploys/proves identity and full manifest regression, and only then removes Vercel `DIRECT_URL`. A missing target URL fails startup/probe; it never selects migration credentials.
...
- New `packages/ai/src/decisions/reconcile.ts` and `retention.ts`; call both from the existing `apps/workers/src/trigger/claim-extraction-batch-drain.ts` schedule rather than adding a Trigger.dev schedule. Its current ten-minute run marks every `pending` row older than its recorded `deadline_at + 2 minutes` as `discarded/process_interrupted` in bounded indexed pages, expires query origins, and uses a database lease to run bounded retention at most once per UTC day. `runDecision` also reconciles the same versioned decision key before insert. A stale row can never be applied; a retry creates a new run linked to the same subjects. Remove the drain's early successful return when no provider batches exist. After the optional batch loop, always run decision reconciliation and retention exactly once, then complete. Continue processing after per-batch failures and leave transiently failed rows retryable with error metadata; mark the scheduled `job_runs` row failed and throw one bounded aggregate error if either drain or maintenance failed, including the no-pending-batches path. No partial/no-work failure may return `{ok:true}` or a successful Trigger run.
...
- Extend `apps/web/app/admin/claims/_actions.ts:updateClaimStatus`, used by both claims pages, to resolve linked new-claim holds. Inside its existing transaction, lock the pending claim and every candidate with `promoted_to_claim_id=claim.id AND status='review_held'`, using a shared adjudication-health helper. Approval revalidates surviving evidence, records human `supported/allow`, resolves the decision review, marks linked candidates `promoted`, records validation results, then approves the claim. Rejection records `unsupported/reject`, resolves review, marks linked candidates `rejected`, records validations, then rejects the claim. Require the claim still be `pending_review`; double-submit is idempotent and concurrent approve/reject permits one winner. Ordinary pending claims retain current behavior. Failed approval revalidation leaves claim/candidate/decision unchanged and visibly reviewable; rejection remains available so no hold is stranded.
...
- Add `live_recall_deliveries` in `packages/db/src/schema.ts` plus the next governed SQL migration, uniquely keyed to the source message/utterance, with states `received→analyzing→no_post|post_ready→send_started→sent` plus `retryable_pre_send|send_unknown|failed_terminal`, lease/token fields, frozen-snapshot/result/draft hashes, bounded error metadata, and timestamps. Add one-delivery-per-source and one-live-intervention-per-delivery constraints; existing message uniqueness alone is not a send fence.
- Add `apps/workers/src/lib/live-recall-delivery.ts` and update `apps/workers/src/lib/recall.ts`. Atomically upsert source message plus delivery; duplicates load/resume state rather than exit. Lease analysis so expired `analyzing` is recoverable. Persist the frozen snapshot, Jev/current-analyzer results, and approved draft before transport. Under the existing channel lock, recheck kill switches, direct-mention behavior, cooldown/rate limits, then atomically reserve `send_started`; reservations and unknown sends count against limits. Commit before HTTP and call Recall once. HTTP 200 uses a fenced transaction to write `sent`, one assistant message, and one intervention. Timeout/reset/5xx or crash after `send_started` becomes `send_unknown`, alerts, and is never automatically resent—at-most-once posting may sacrifice a message rather than duplicate one. A 429 may retry only per `Retry-After` before/through the explicit retryable state; definite 4xx is terminal. Re-enable bounded task retries and add the existing-schedule sweeper for expired analysis leases and `send_started` reconciliation.
- Fix `apps/web/app/api/teams/live/recall/route.ts` to await `triggerTask` and return non-2xx when dispatch fails so Recall retries; do not acknowledge 202 when nothing was queued. If the implementation instead persists an ingress inbox before 202, that inbox and its sweeper must be part of the same step and verifier.
...
- The verifier injects crashes after source persistence, analysis, `post_ready`, before/after `send_started`, after Recall 200 before database finalization, and after final commit; it also covers 100 concurrent duplicate deliveries, two utterances racing one channel, stale-lease takeover, Jev fallback, dispatch failure, 429, timeout, 5xx, and 4xx. Every case proves deterministic resume or no-resend behavior, at most one Recall call after the send fence, one assistant record, and one intervention.
...
- Schedule-capacity/failure verification counts `schedules.task()` exports before/after, requires the total to remain nine, and proves `claim-extraction-batch-drain` invokes decision maintenance without weakening its original drain. Add `apps/workers/src/__verify__/jev-decision-maintenance-drain.ts` and a package script. Cover zero batches with maintenance success, zero batches with maintenance failure, healthy and transiently failed batches plus maintenance, exactly-once maintenance, and aggregate failure behavior. The healthy batch completes, failed rows remain retryable with bounded metadata, decision maintenance still runs, the scheduled `job_runs` row fails, and the task rejects/non-zero. Record a successful worker deployment/capacity result before completion.
