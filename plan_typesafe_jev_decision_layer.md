...
Publication gate: **not approved**. Exact-head review `20260920T175500-962763-18364` rejected commit `7d11af2`; this branch contains a partial documentation-only repair that must be completed, committed, pushed, and independently re-reviewed before a PR is opened. No implementation step is authorized from this draft.
...
- Chat output: `apps/web/app/api/chat/route.ts:223-315,515-663` now builds bounded cross-domain approved context, enforces known citation IDs, records coverage omissions, generates, and persists the answer; no independent semantic grounding judge runs before persistence. `apps/web/lib/business-answer-{retrieval,context,policy}.ts` is now part of this load-bearing path. Issue #15 closed on 2026-09-20 with only partial live acceptance: evidence delivery/citation identity passed, but coherent rule/exception/handoff reconciliation did not. Steps 9A/9B must read that final evidence, preserve the proven behavior, and treat evidence reconciliation as an explicit baseline limitation rather than claiming the predecessor proved holistic comprehension.
...
- Enable row-level security on all nine new `oracle_decision_*` tables in the foundation migration. Follow the repository's existing intelligence-table pattern: deny anonymous access; authenticated non-admin users can neither select nor mutate; authenticated admins may select the bounded audit/control data needed by the admin UI; browser roles never insert/update/delete. Revoke direct mutation grants from `anon`, `authenticated`, and `service_role`; application writes use narrowly scoped `SECURITY DEFINER` database functions owned by the migration owner, with fixed `search_path`, explicit admin/server authorization, row locks, and the same validations described here. Service-role possession is not permission to bypass the controls. Keep the admin check in RLS as well as in server actions so direct database access cannot bypass it.
- Add database enforcement for immutable/append-only promises: identity/policy/version fields on data policies, spend epochs, decision runs, and canary reservations cannot update; adjudications and control events reject update/delete; successful outcomes and subject sets are frozen; policy revocation, spend reservation/settlement/epoch close, decision finalization, source erasure, and control transitions are separate guarded functions that can modify only their named mutable columns and write the required event atomically. Composite uniqueness on `oracle_decision_adjudications(id, decision_run_id)` plus composite FKs `(supersedes_id, decision_run_id)` and `(current_adjudication_id, oracle_decision_runs.id)` ensures both the superseded row and current head belong to the same decision run. Privileged direct-write tests use the application `service_role` and must be rejected; only the named functions succeed.
...
- Add admin-only classification controls in `apps/web/app/admin/documents/page.tsx` and `_actions.ts` that select one or more allowlisted **company-data** classes, require the current data-policy version and a bounded reason code, and atomically write the document plus a control event carrying its typed document FK and bounded before/after class sets/reason. `public_synthetic` is not a selectable document class: a database check rejects it in `documents.decision_data_classes`, the server action rejects it before SQL, and direct authenticated/admin/service-role write tests prove it cannot be assigned. Reclassification cannot make an active Jev call retroactively valid; future calls load the stored current value. Bulk auto-classification is out of scope.
...
- Seed those settings in the foundation migration. Reserve every `jev_*` setting key at the database and generic API boundaries: `apps/web/app/api/admin/settings/route.ts` must reject reads/writes that attempt generic mutation of that namespace, and a `BEFORE INSERT/UPDATE/DELETE` trigger on `settings` rejects reserved-key mutation by browser/admin/service-role sessions. Only the fixed-`search_path` `SECURITY DEFINER` Jev transition function owned by the migration owner may change the namespace, and it accepts a typed use-case/action rather than an arbitrary key/value. Add an admin-only “Jev decision controls” section under `apps/web/app/admin/settings/page.tsx` with a focused server action/component that calls only that function: `off→shadow` requires pinned requested-model/pack/threshold/active-data-policy versions; completed shadow evaluation records the exact provider-returned resolved model in the evidence artifact. `shadow→active` requires that durable `<key>_approved_resolved_model`, a committed held-out artifact URL, and approving independent-review commit all bound to the exact configuration fingerprint. Any requested/approved-resolved model, pack, threshold, or data-policy change is one locked transition that first forces `active→shadow`, clears approved-resolved-model and prior activation evidence/review fields, increments activation generation, and writes a control event; it cannot edit an active configuration in place. For every active response, the request model recorded on the run must match configured requested model and the provider-returned resolved model must match stored approved resolved model; either mismatch discards the answer and atomically demotes that use case to `shadow`. Every manual transition, automatic health fallback, emergency off, or policy-revocation demotion uses the same lock/generation/event path. Reactivation always needs new exact-fingerprint held-out/review evidence and a fresh generation; no environment-only, direct-settings, or dashboard bypass exists.
...
- Migration/schema verification proves the dedicated tables, constraints, indexes, generated types, reserved-settings trigger, same-run composite adjudication FKs, immutable/append-only triggers, and function/grant boundaries match this contract. Direct `service_role` attempts to mutate policy/cap/append-only rows or any `jev_*` setting fail; each permitted transition succeeds only through its named database function and records its event.
...
- Keep `searchWithRetrievalPlan`, both its vector and `_searchFallbackTsvector` branches, and their round-robin candidate merge as deterministic candidate generation. In `retrieveBusinessAnswerContext`, finish every search pass and deduplication first, then load eligible relationships, their complete support sets, and the exact evidence/source rows intended for Jev. Only after that enrichment may one complete `chat_answer` Jev decision run; it never runs once per search pass. Missing provenance or enrichment returns the byte-identical baseline order with zero Jev calls.
...
- Score each enriched claim once, then apply that score only within its original search-pass bucket before the existing round-robin merge so domain coverage survives adversarial scores. Score relationships only as atomic bundles with their complete support set. Refactor `buildBusinessAnswerContext` to accept explicit direct-claim and relationship orders; Jev may reorder but never filter, split a relationship from its premises, change omission flags, alter citation membership, or add candidates. Timeout/failure restores byte-for-byte baseline ordering/context, and non-chat callers never reach this decision path.
...
- Add `packages/ai/src/__verify__/jev-retrieval-rerank.ts` and `evals/jev/retrieval-rerank.md` with vector/fallback parity, an invocation matrix for all five callers, multi-message erasure, recall@k, precision@k, downstream answer support, latency, tokens, and cost. Prove transport cannot start before complete relationship/support/evidence enrichment; exactly one decision covers the bundle; vector/fallback sources produce identical subject completeness; round-robin domain representation survives adversarial scores; multi-support relationships remain atomic; missing evidence/source linkage yields zero calls and exact baseline output; every source deletion and permission-scope change invalidates reuse; MCP, contradiction, live Recall, and test callers make zero rerank calls.
...
- Issue #15 is closed, so its ownership wait is over; before Step 9A or 9B, import its final partial-acceptance evidence into the baseline. Do not reopen or silently absorb its failed comprehension scope: Jev reranking/grounding must preserve proven citation identity and separately measure the documented rule/exception/handoff reconciliation gap.
