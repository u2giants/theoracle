...
Publication gate: **not approved**. Exact-head review `20260922T215441-22913-5675` rejected commit `c09f5bb`; this documentation-only draft is repairing its six findings and must be committed, pushed, and independently re-reviewed before a PR is opened. No implementation step is authorized from this draft.
...
- Add committed, machine-checked `config/runtime-db-access.json` before role creation. Inventory every direct or indirect production use of `getDirectDb`, `getPooledDb`, raw `postgres()`, and `createServiceRoleClient`, including transitive package helpers. Each entry is classified `vercel|trigger|migration_admin|test_only` and records file/export, caller surface, runtime credential, tables/functions, `SELECT|INSERT|UPDATE|DELETE|EXECUTE`, raw/dynamic SQL, and a regression-test ID; no `shared`/`unknown` entry passes. A CI scanner fails on any unmanifested call site, relation/routine/operation, or dynamic SQL fragment. Generate/rehearse grants from this inventory, then compare effective PostgreSQL privileges for both runtime roles against it on a fresh database and current-production snapshot clone; missing **or extra** privilege fails.
- Enable row-level security on all nine new `oracle_decision_*` tables in the foundation migration. Deny anonymous access; authenticated non-admin users can neither select nor mutate; authenticated admins may select bounded audit data but browser roles never directly mutate. Revoke direct `INSERT/UPDATE/DELETE` from `PUBLIC`, `anon`, `authenticated`, `service_role`, both runtime roles, and generic application roles. Grant runtime roles only manifest-required existing-app privileges plus the exact runtime Jev functions needed to create/finalize decisions, reserve/settle spend, reconcile, and force safety-reducing `active→shadow|off`; they can never activate, widen policy, classify a document, or create/raise an epoch. Human activation, policy widening/revocation, document classification, and spend-epoch creation use cookie-bound Supabase RPC, not shared Drizzle. The fixed-path owner function is granted only to PostgREST `authenticated`, requires non-null `auth.uid()`, resolves it through `employee_identities`, verifies an active admin employee, and derives the actor internally; it accepts no employee/role assertion. `oracle_web_runtime`, `oracle_worker_runtime`, `anon`, and `service_role` receive no `EXECUTE` on human-control functions. The boundary protects normal browser/API/RPC and least-privilege runtime access; the guarded migration/schema owner remains trusted deployment authority, and the plan does not claim resistance to malicious schema-owner use.
...
- Split foundation delivery into ordered outcomes. **Step 1A (schema, manifest, and role boundary only):** commit the complete access manifest/scanner, merge the reviewed migration/schema/role topology with no Jev reader, run the guarded production migration, and verify journal, identities, ownership, exact manifest grants, and RLS. **Step 1B (dependent client/code):** only after 1A proof, merge the runtime clients, decision client, controls, pages, banner, and worker code with real-database integration tests under both new roles. Cut over **one deployed surface per later session/live outcome**: run that surface's complete manifest regression group, deploy its read-only identity/privilege probe through the actual credential, switch it, rerun the group and business smoke, then remove the migration credential from that platform. Trigger and Vercel cutovers are separate outcomes; any unclassified call or changed existing behavior blocks removal. No commit that queries a new decision table merges before migration proof.
...
- Direct-access tests exercise anonymous, signed active-admin/non-admin/disabled/expired JWTs, Supabase `service_role`, `oracle_web_runtime`, and `oracle_worker_runtime`: browser reads follow bounded policy, direct Jev mutation fails, and human-control RPC succeeds only for the signed active admin with actor derived from `auth.uid()`. Spoofed employee IDs, null identity, and direct runtime/service-role calls fail. Runtime functions perform only their enumerated safety/runtime transitions. Fresh-install tests create the real topology. Real SQL integration tests cover every access-manifest regression ID, including auth/linking; chat/messages; admin settings/catalog/documents/claims; MCP and Teams/Recall webhooks; extraction, document ingestion, contradiction, synthesis, transcript, batch-drain, and maintenance workers, with external providers mocked. Post-cutover probes prove the expected distinct role, no owner/`BYPASSRLS`, exact manifest privileges, and migration credential absence.
...
- The current entity selector does not persist `model_runs`. Represent this use case with `oracle_decision_subjects.entity_query_key`, one typed `entity_id` for **every** serialized registry candidate, and every contributor message plus the chat query origin. Extend the origin with channel, locale, permission kind/scope hash and subjects with frozen `subject_hash`/ordinal. One pre-transport transaction verifies current membership, message/channel roles and hashes, candidate registry hashes, and exact subject counts; creates origin, spend reservation, pending run, and all origin/message/entity/query-key subjects; then commits. Only after commit may TypeSafe run. A mocked transport on a separate DB session must observe the committed complete set. Any mismatch/conflict/insert failure rolls back and returns current behavior with zero calls. The input hash covers ordered contributor IDs/included hashes, origin/scope, locale, ordered candidate IDs/serialized hashes, and config versions. Deletion/mutation invalidates the whole run. Store the current selector's bounded IDs, route/model, attempt, latency, and usage in comparator fields; never invent a model-run link.
