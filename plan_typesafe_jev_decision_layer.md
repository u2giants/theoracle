...
Publication gate: **not approved**. Exact-head review `20260922T221548-27928-9129` rejected commit `5059615`; this documentation-only draft is repairing its capture-edit finding and must be committed, pushed, and independently re-reviewed before a PR is opened. No implementation step is authorized from this draft.
...
- In `packages/db/src/schema.ts`, the generated migration, and listed hand-written SQL, add `messages.extraction_directive` (`normal|force_extract`, default `normal`), nullable capture source/time/requester, plus `content_version` and `content_hash`; add the batch snapshot/status fields and normalized membership below. Constraints require capture fields null for `normal`; `force_extract` requires a user message whose requester equals its employee. A `BEFORE UPDATE` trigger rejects every capture-field change and rejects content/version/hash changes unless `current_user=oracle_jev_owner` inside the named edit function, so RLS is not asked to compare old/new values. Add typed fixed-path `oracle_create_channel_message` and `oracle_update_user_message_content` functions. Both authenticate `auth.uid()` through employee/channel membership; create derives capture fields from typed intent/exact command, while edit accepts only message ID, expected content version, and new content and preserves capture fields. Revoke direct UPDATE of content/capture/version/hash columns from authenticated/service/runtime roles; retain only access-manifest-required status-column grants for workers, and grant authenticated users the edit RPC.
...
- Typecheck and the verifier pass in both dispatch modes. Handler/RPC tests cover unauthenticated/nonparticipant/inactive channel, malformed/privileged fields, empty/exact command, composer intent, ordinary messages, actor spoofing, and expected-version conflict. Direct authenticated/service/runtime SQL cannot set/change capture fields or update content; only signed create/edit RPC works. Barrier races cover edit versus pending/negative/provider-complete finalization: either edit requeues and stale finalization fails, or completed evidence makes edit fail with zero mutation. Editing a forced pre-extraction message preserves force and produces zero suppression calls; post-extraction edits are rejected. UI tests cover toggle reset/restore, badge, Realtime/initial parity, and conflict guidance. Imports, Teams/assistants, quoting, and near matches remain normal.
