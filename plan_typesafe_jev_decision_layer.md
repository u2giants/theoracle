...
Publication gate: **not approved**. Exact-head review `20260920T175500-962763-18364` rejected commit `7d11af2`; this rebased documentation-only draft contains source-backed repairs for all fifteen findings and must now be committed, pushed, and independently re-reviewed before a PR is opened. No implementation step is authorized from this draft.
...
- Enable row-level security on all nine new `oracle_decision_*` tables in the foundation migration. Follow the repository's existing intelligence-table pattern: deny anonymous access; authenticated non-admin users can neither select nor mutate; authenticated admins may select bounded audit/control data but browser roles never mutate. Revoke direct `INSERT/UPDATE/DELETE` grants from `anon`, `authenticated`, and `service_role` on every table. Application writes use named, typed `SECURITY DEFINER` functions owned by the migration-owner role, with fixed `search_path`, explicit admin/server authorization, and row locks; inside them `current_user` is the migration owner while `session_user=service_role` remains auditable. A direct service-role statement never qualifies merely by setting session metadata. Keep the admin check in RLS and the function, so API or direct database access cannot bypass it.
...
- In `packages/db/src/schema.ts`, the generated migration, and a listed hand-written SQL migration, add `messages.extraction_directive` (`normal|force_extract`, default `normal`), the new batch statuses/snapshot fields below, and normalized authoritative window membership. Merge this backward-compatible schema alone; run `pnpm db:migrate`, prove journal/constraints/indexes/backfill on production, and smoke unchanged workers. No worker/web code may import or query the new schema before that proof.
...
- Add an authenticated server-owned `/capture ` command and “Save as knowledge” action that persist `force_extract` at message creation. Callers cannot supply the database value directly; near matches, quoted commands, arbitrary metadata, imports, Teams, assistants, and ordinary messages remain `normal`. Include the directive in input identity. Any window containing `force_extract` bypasses Jev before transport in shadow/active modes; edits re-run the deterministic parser and invalidate old decisions.
...
- Typecheck and the new verifier pass in both dispatch modes. Exact command/UI cases set `force_extract`; near matches, quoting, metadata spoofing, imports, and direct writes cannot. Flagged windows make zero Jev suppression calls; overlap/edit/retry/replay preserve forced extraction.
...
- Reuse the pair-aware helper immediately before provider dispatch to close sweep/task races. Existing contradictions/current negative dispositions suppress only their exact pair. Policy/model/pack/threshold/hash drift invalidates only that pair's disposition and reevaluates it once. Add canonical-pair indexes/query-plan proof so later sweeps avoid processed pairs without fake contradiction rows.
