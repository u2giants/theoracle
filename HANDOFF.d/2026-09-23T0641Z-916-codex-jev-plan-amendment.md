...
- The amendment splits M0 into independently buildable M0P, M0A–M0D, M0W, M0R, M0I, M0E, and M0F outcomes. M0P first publishes central class `migration-release` from `popcre/ai-devops`; M0A pins it and adds Oracle's union of reviewer-safety, shared-db, infrastructure, deployment, code, and prose gates. M0W lands the disabled exact workflow, M0R applies the no-bypass main ruleset, M0I provisions workflow-bound GCP credential/MAC infrastructure, and M0F alone may perform the production database-write outcome.
...
- A live GitHub API check on 2026-09-23 returned 404 for `branches/main/protection`; the earlier “protected main” assumption was false. The repair adds M0W's checked-in probe-only workflow/contract and M0R's separately reviewed no-bypass ruleset apply/proof before M0I creates any cloud trust. This self-audit finding is not accepted until the next exact-head review approves the complete amendment.
...
- A repository secret cannot be honestly described as confined to one workflow, and a 1Password service account is vault-scoped rather than item-scoped. Production now uses no GitHub-held production secret: exact-workflow OIDC obtains short-lived access to two Secret Manager resources and one non-exportable KMS MAC key. Table-driven CEL-condition fixtures prove sibling/wrong-context refusal; the live positive probe and Cloud Audit Logs prove the exact accessing identity without pretending an unexecuted negative identity was live-tested. Because that probe reads secret payloads, it needs a separate production gate and independent exact-scope approval even though it never connects to the database.
...
- GitHub repository governance is a prerequisite trust root too: the cloud identity is not provisioned while `main` remains unprotected.
...
- Human-managed source secrets belong in 1Password vault `vibe_coding`; M0I later creates two narrow Secret Manager delivery replicas and a non-exportable KMS key through protected pipes and independent infrastructure approval. No secret is needed to publish this documentation amendment.
