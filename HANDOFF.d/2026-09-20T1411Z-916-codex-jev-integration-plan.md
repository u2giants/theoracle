...
- Exact-head review `20260923T052049-36-18308` rejected commit `f406091` because M0 depended on Step 1A's not-yet-created runtime manifest, aggregator wording conflicted over two versus three jobs, and the old-app compatibility script escaped task gates. The correction gives M0 its own exhaustive source-hashed old-app smoke manifest, makes Step 1A import it later, requires all three CI dependencies consistently, and classifies the compatibility script as shared-db. None is accepted until the next exact-head review approves it.
- Exact-head review `20260923T052659-1881-21661` rejected commit `a4970a3` because proof coverage was not deny-by-default across cluster objects, scratch storage could enter repos/sync paths, control tables lacked a documented schema exception, optional migration 99 had no ledger lifecycle, log scanning could pass before ingestion, and M0 was too broad for one session. The correction inventories/fingerprints or rejects every database/cluster class, fixes a canonical non-reparse ProgramData root, documents/tests the bootstrap exception, adds `deferred` activation, waits for a post-operation log watermark, and splits M0A–M0E into separate reviewed milestones with only M0E writing production. None is accepted until the next exact-head review approves it.
- Exact-head review `20260923T054041-1959-7768` rejected commit `661e8f8` because the M0 milestone names still had circular dependencies and only whole-M0 tests. The correction makes A pure and non-invasive, B fresh-only with a hard no-proof-engine production refusal, C complete but production-disabled, D the non-production platform/rehearsal cutover, and E a tiny reviewed activation PR; each owns a green command and merge artifact using only prior milestones. None is accepted until the next exact-head review approves it.
- Exact-head review `20260923T055059-1224-2518` rejected commit `7117369` because M0B still invoked proof-dependent real migrations, read-only production export lacked an executable production gate, the rehearsal artifact was not verifiable, and deployment files were incorrectly treated as shared-db. The correction splits M0A–M0F by native task class, confines M0C to synthetic state-machine actions until M0D installs proof, requires a signed exact-SHA production-read approval before M0E connects, defines a signed 30-day GitHub artifact plus verifier, and reserves M0F for activation. None is accepted until the next exact-head review approves it.
...
2. Execute **M0A only**: the reviewer-safety rulebook/decision/task-classification contract and its table-driven gate test; add no implementation, manifest/workflow change, or database access. Successors take M0B through M0F in order, each in its native class; no session bundles milestones or adds Jev schema.
3. After each milestone update its STATUS row, successor handoff, and issue #14 with that commit/check evidence. M0E owns the separately approved read-only production rehearsal artifact; M0F alone records production baseline/bootstrap/idempotency proof. Keep issue #14 open.
