...
2. Execute **M0A only**: pure inventory/order/tokenizer/scanner/task-gate code and `verify:m0a`; do not alter live migration/seed commands or touch a database. Successor sessions take buildable M0B, M0C, M0D, then M0E; no session bundles milestones or adds Jev schema.
3. After each milestone update its STATUS row, successor handoff, and issue #14 with that commit/check evidence. M0E alone records the production baseline/bootstrap/idempotency artifact. Keep issue #14 open.
