...
| `default_macro_route` | `openai/gpt-4.1-mini` | Primary model for the macro/holistic layer (`source-outline`, `macro-relationship-extraction`, `source-coverage-audit`). SEPARATE from the extraction/vision/general slots. Requires STRICT structured output that can handle deep nested schemas — **Gemini 2.5 flash/pro 400 on these schemas ("constraint too complex")**, so use an OpenAI strict-json-schema model. Chosen at Admin → Settings → "Macro understanding model" (migration `83`). |
| `model_pool_macro` | `["openai/gpt-4.1-mini","openai/gpt-4.1","google/gemini-2.5-pro"]` | Ordered fallback chain for the `macro` slot (the ONE auxiliary slot with a pool — resilience for the holistic layer). Must contain the `default_macro_route` primary. Editable in Admin → Settings → Model Pool via the Macro column. Migration `84`. |
| `default_workflow_read_route` | `anthropic/claude-sonnet-5` | Macro-first source workflow reader. `document-ingestion` awaits this pass before extraction, and the standalone Trigger task id is `source-workflow-read`. The output schema is flat nodes/edges/lanes/paths and is persisted to `source_workflow_maps`. |
| `model_pool_workflow_read` | `["anthropic/claude-sonnet-5","google/gemini-2.5-pro","openai/gpt-4.1"]` | Ordered fallback chain for the workflow-read slot. Stage gates require a bake-off before treating the seeded order as final. |
...
| `macro_max_lens_model_calls_per_document` / `macro_max_lens_groups_per_document` / `macro_max_lens_estimated_input_tokens` | `16` / `20` / `64000` | Raised from `4`/`8`/`32000` (migration `84`) so coverage-first lens fan-out can cover every workflow stage. Selection is round-major (every group gets its top lens before any gets a second); terminal stages are no longer dropped. `document-lens-budget.ts`. |
