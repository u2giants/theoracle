...
| `extraction_dispatch_mode` | `'sync'` | `'sync'` \| `'batch'`. When `'batch'`, claim-extraction submits requests through the provider Batch API (OpenAI or Vertex; D14) for ~50% off pricing with a 24-hour SLA, drained by a separate cron. Provider must implement `submitBatch` / `retrieveBatch`; otherwise the worker falls back to sync. **Worker integration is deferred — flag is recognized at the DB level but not yet wired into the worker. See HANDOFF.md.** |
