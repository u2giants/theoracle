...
| `model_pool_interview` | `[]` | Admin-curated shortlist for the Interview stage picker. `string[]` of "provider/modelId" IDs from the discovered catalog (`model_capabilities` table). Empty = `/api/admin/models` returns the full catalog. Interview filters to tools + structured output + vision + context >100K. |
| `model_pool_extraction` | `[]` | Same, for the Extraction stage. Extraction filters to structured output + context >100K; it intentionally does **not** require vision because uploaded images are first transcribed by `default_vision_route`, then Extraction receives text. |
| `model_pool_synthesis` | `[]` | Same, for the Synthesis stage. Synthesis filters to context >400K + structured output + reasoning + output-cap support. |
| `default_general_purpose_route` | _(unset)_ | Optional: model id for one-off internal jobs (taxonomy cluster naming, future classifier fallbacks). An **auxiliary model** (see `packages/ai/src/routes/auxiliary.ts`), picked at `/admin/settings`. Not yet wired into worker code paths. |
| `default_vision_route` | `vertex_gemini_2_5_flash_extraction_primary` (seeded) | Vision model used by `apps/workers/src/trigger/document-ingestion.ts` to transcribe uploaded images to text (Pass 1) before claim extraction. An **auxiliary model** resolved via `resolveAuxiliaryRouteFromSettings(db, 'vision')`; falls back to the Gemini route when unset. Chosen at `/admin/settings` → "Image vision model" (picker filtered to vision-capable models). Inference is provider-direct (Gemini via `@google/genai`), never OpenRouter. **Pick a vision (image-reading) model, NOT an image-generation model.** Prod was found set to `google/gemini-3.1-flash-image-preview` (an image-*generation* preview model), which transcribes diagrams inconsistently/truncated; the seeded Gemini 2.5 Flash route is the reliable default. See AGENTS.md §10 "Diagram / flowchart image ingestion". |
