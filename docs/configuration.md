...
| `default_transcript_summary_route` | `qwen/qwen3.6-flash` | Dedicated auxiliary model route for on-demand Teams transcript picker summaries. `teams-transcript-summary` resolves this slot first; if the slot has no usable configured candidates, it loudly logs and falls back to `general` as a compatibility path. |
| `model_pool_transcript_summary` | `["qwen/qwen3.6-flash","qwen/qwen3.7-plus"]` | Ordered fallback chain for Teams transcript picker preview summaries. Seeded to the bake-off winner and same-provider Qwen fallback. |
