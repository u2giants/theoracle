...
| `default_macro_route` | `openai/gpt-4.1-mini` | Primary model for the macro/holistic layer (`source-outline`, `macro-relationship-extraction`, `source-coverage-audit`). SEPARATE from the extraction/vision/general slots. Requires provider-enforced STRICT structured output that can handle deep nested schemas. OpenAI strict `json_schema` is the intended lane. Qwen/DeepSeek are blocked because they provide JSON mode + Zod validation, not strict schema; Gemini 2.5 flash/pro are blocked for the complex macro slots because they can 400 with "constraint too complex." Chosen at Admin → Settings → "Macro understanding model" (migration `83`). |
| `model_pool_macro` | `["openai/gpt-4.1-mini","openai/gpt-4.1","google/gemini-2.5-pro"]` | Ordered fallback chain for the `macro` slot (the ONE auxiliary slot with a pool — resilience for the holistic layer). Must contain the `default_macro_route` primary. Editable in Admin → Settings → Model Pool via the Macro column. Runtime capability enforcement rejects providers that do not meet the strict macro requirements even if a stale row remains in the setting. Migration `84`. |
