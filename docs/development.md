...
- `packages/db/src/verify-identities.ts` — GAP-10 identity cleanup gate. Run `pnpm --filter @oracle/db verify:identity-cleanup -- --contract-only` for the network-free fixtures, owned-reader scan, and exact migration checks. The pre-drop credentialed live audit and rollback-compatible authentication proof passed on 2026-07-29. Raw migration 98 is now the only authorized deprecated-column drop. Do not rerun the live pre-drop audit after migration 98 removes those columns; verify the release, migration state, and protected authentication instead.
