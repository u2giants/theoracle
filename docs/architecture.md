...
The macro-first layer starts with a source workflow read. During document ingestion, after parsing/chunking/embedding and before claim extraction, `document-ingestion` awaits `generateSourceWorkflowMap()` (also exposed as the `source-workflow-read` Trigger task) when `map_directed_extraction_enabled=true` (the default). The reader loads all persisted `document_chunks` in order, resolves the `workflow_read` model slot, and writes one immutable `source_workflow_maps` row with flat `nodes_json`, `edges_json`, `lanes_json`, and `paths_json`. Validation is deterministic: node/edge endpoints must resolve, each node/edge quote must validate against the cited chunk, paths can only reference existing nodes, and dropped elements are recorded in `validation_json`. A failed reader writes a failed map/job-run and marks `documents.macro_health='map_failed'`; by default (`require_workflow_map_for_ingestion=false`) ingestion continues through blind extraction with a degraded processing note. If that setting is true, reader failure still fails the document. Degraded maps remain usable but surface as degraded health.
