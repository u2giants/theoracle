...
- `employee_identities` is the only identity source in the current schema. The rollback-compatible release and protected-login proof passed on 2026-07-29. The local forward change removes the old `employees.auth_user_id`, `employees.auth_provider`, and `employees.auth_provider_subject` fields through rerun-safe raw migration `98_drop_deprecated_employee_identity_columns.sql`; production migration and post-drop release/authentication proof are still pending. The offline guard is `pnpm --filter @oracle/db verify:identity-cleanup -- --contract-only`. See `DECISIONS.md` D2.multi-identity.
...
- **Legacy identity columns are removed by raw migration 98.** All owned runtime readers use `employee_identities`. The migration is implemented locally but is not yet applied to production; post-drop deployment and protected-login proof are still required.
