...
| Settings / model config | `apps/web/app/admin/settings/model-pool` — per-stage pool checkbox table (Interview / Extraction / Synthesis columns) backed by the persisted `model_capabilities` Postgres table. Data source: `packages/ai/src/model-capabilities/sources/openrouter.ts` parses openrouter.ai/api/v1/models on admin refresh. Pricing comes from OpenRouter's `pricing.{prompt,completion}` × 1M; capability flags from `architecture.input_modalities` + `supported_parameters`. `/api/admin/model-catalog`: GET reads the table, POST refreshes from OpenRouter and upserts. `/api/admin/models?stage=<>`: returns the per-stage pool (`settings.model_pool_<stage>`) or full catalog if pool is empty. Workers resolve their route via `resolveModelRoute(modelIdOrRouteId, role)` in `packages/ai/src/routes/resolve.ts`, which accepts both catalog `routeId`s and `provider/model` strings and synthesizes a route for any non-catalog model. Three role-setting keys (`default_interview_route`, `default_extraction_route`, `default_synthesis_route`) feed all six production callers; a fourth `default_general_purpose_route` exists for internal one-off jobs but is not yet wired to any caller. |
