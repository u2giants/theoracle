...
`/admin/ai/extraction-ab` is a non-promoting A/B/C review surface for those same approved revisions. It compares the existing Gemini 2.5-era claim, a fresh `google/gemini-3.1-flash-lite` extraction, a fresh `qwen/qwen3.7-max` extraction, and the human revision, then stores the reviewer's winner in `claim_extraction_ab_tests`. These eval outputs are never inserted into `claims`; they are only for choosing better extraction models/prompts.
