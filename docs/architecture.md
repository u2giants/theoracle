...
The macro-understanding layer starts with provisional document outlines. The `source-outline` Trigger.dev task reads persisted `document_chunks`, calls the admin-selected `macro` auxiliary model through `OracleAIClient`, and stores a `source_outlines` row plus `source_groups`, `source_group_items`, and inspectable chunk refs. It then acts as the document macro orchestrator: a deterministic budget gate selects high-yield source-group/lens pairs and dispatches `document-lens-extraction` child tasks. Lens jobs no longer each launch their own macro followups; the final completed/skipped-existing lens job claims a one-time dispatch latch on `source_outlines.budget_json` and triggers `macro-relationship-extraction` once. The macro worker then triggers `source-coverage-audit`, so coverage sees the relationships that were just written. Admins can manually generate or regenerate an outline from `/admin/documents`.

Outlines are guidance only. They help later extraction understand whole-source shape, handoffs, branches, acronyms, and likely extraction lenses, but they are not evidence and do not create claims. Document ingestion only injects the latest provisional outline when `settings.macro_outline_injection_enabled=true`; even then, `claim_evidence` must still quote one persisted `document_chunk`, and the existing deterministic quote validator remains the trust boundary. Lens passes use `batch_type='document_lens_group'`, record the active lens and source group in `raw_candidate_json`, and run the same quote, taxonomy, sensitivity, and promotion gates as the broad document pass.
...
**Model routing + health (2026-07-03).** The three macro workers resolve a dedicated `macro` auxiliary slot (`default_macro_route` primary + fallback pool `model_pool_macro`) — NOT the extraction or `general` route. The macro relationship/coverage schemas are deeply nested and need a strict-json-schema model that can handle complex schemas: **Gemini 2.5 flash/pro reject them (`400 "constraint too complex"`)**, so the primary is an OpenAI strict-schema model (`openai/gpt-4.1-mini`) with Gemini as fallback only (Gemini is fine for the simpler source-outline schema). See AGENTS.md §11. All three workers write `job_runs`, and their outcome is recorded on `documents.macro_health` (`not_applicable | pending | complete | degraded | failed`, `apps/workers/src/lib/macro-health.ts`) and rendered in Admin → Documents so a document whose holistic layer failed cannot read as a green `complete`.
