...
- Deprecated columns `employees.auth_user_id`, `employees.auth_provider`, `employees.auth_provider_subject` remain as rollback placeholders. All owned runtime readers use `employee_identities`. The offline guard is `pnpm --filter @oracle/db verify:identity-cleanup -- --contract-only`; that command includes its network-free fixtures. The credentialed production gate passed on 2026-07-29 with 38 employees, zero non-null values in all three deprecated columns, and no external or inbound dependency blockers. Only the expected local unique constraint and index were reported. Every deprecated-column drop remains blocked until a rollback-compatible release authenticates successfully. Then removal must use a new forward-only raw migration that sorts after `40_employee_identities_data.sql`. See `DECISIONS.md` D2.multi-identity.
...
- **Deprecated columns on `employees` (`auth_user_id`, `auth_provider`, `auth_provider_subject`) are still present only for rollback.** All owned runtime readers use `employee_identities`. Their final removal is gated by the aggregate live data/dependency audit and proof that the rollback-compatible release authenticates. New code must read identities through `employee_identities`, not these columns.
...
After extraction, a DB-free shape registry deterministically reconciles each shape's primary map elements/relations to surviving claim refs. Missing primary refs upsert stable `gaps` rows with `gap_type='model_coverage'`; repeated reconciliation reuses the same UUID and covered findings become stale. These rows are administrative model-quality findings, never employee questions: employee retrieval and lull-interjection queries explicitly exclude them, and the current admin gaps surface only supports resolving or staling them. Source reads are also bounded before dispatch by configurable call, estimated-token, estimated-cost, repair, and concurrency limits; an over-budget source fails loudly and persists its budget snapshot in map/job diagnostics.
