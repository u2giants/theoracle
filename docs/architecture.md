...
Legacy provisional outlines and lens fan-out still exist for historical rows and manual admin actions during the staged migration, but document ingestion no longer fires `source-outline` after extraction. Stage 3 of `MACRO_FIRST_REDESIGN.md` deletes lens fan-out and the old outline-injection flag path once map-directed extraction/dedup is proven.
