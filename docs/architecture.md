...
1. Resolve a slot through `resolveRouteCandidates(db, slot)`, which reads the primary route setting,
   the approved pool for pipeline and auxiliary slots, optional reasoning effort, and capability
   metadata.
2. Get back an ordered candidate chain. Pipeline and auxiliary slots try the admin-selected primary
   first, then the remaining models in that slot's approved ordered pool. An unset primary fails
   loudly.
...
| Cache strategy                           | Explicit `cache_control` markers on stable prompt blocks and reusable conversation prefixes. | Automatic prefix caching with retention hints; reads reported cached tokens. | Implicit cache plus persisted explicit `cachedContent`, including guarded file-backed inputs for oversized artifacts. | Does not create explicit cache resources; normalizes `cachedContentTokenCount` when Gemini reports it. | Provider-managed automatic prefix caching; reads `prompt_cache_hit_tokens`. | Provider-managed implicit prefix caching only. Explicit markers and Responses session state are disabled pending measured proof. |
| Reasoning effort param                   | `thinking.budget_tokens`; thinking forces `temperature: 1`. | `reasoning_effort` for supported models; off omits it. | `thinkingConfig.thinkingBudget` (off=0, low=1024, medium=8192, high=24576). | Gemini 2.x uses `thinkingBudget`; Gemini 3.x uses `thinkingLevel`, selected from route metadata. | None passed. DeepSeek reasoning is provider-controlled. | `enable_thinking` plus optional `thinking_budget`; structured JSON calls force thinking off. |
| Usage normalization (into `OracleUsage`) | Input, output, cache-read, cache-write, and reasoning tokens from Anthropic usage. | Input, output, cached input, and reasoning tokens from OpenAI usage. | `usageMetadata` prompt, candidate, cached-content, and thought token counts. | `usageMetadata` prompt, candidate, cached-content, and thought token counts. | OpenAI-like input/output plus DeepSeek-specific `prompt_cache_hit_tokens`. | OpenAI-like input/output, cached input, cache creation, and reasoning token details when reported. |
| Multi-turn messages                      | Uses the `providerOptions.messages` override. | Uses the `providerOptions.messages` override. | Transforms text, image, and PDF parts into Vertex contents while preserving non-cached attachments. | Transforms provider-neutral message content into Gemini contents. | Uses the OpenAI-compatible messages override. | Uses the OpenAI-compatible messages override. |
| What it CAN'T do today                   | Streaming with tools end to end is not wired. | Adapter-native vision input remains limited. | Oversized cross-provider inline fallback is intentionally bounded; required file-cache preparation fails clearly. | No tracked Batch adapter path. | No tracked Batch path, provider-enforced arbitrary JSON Schema, or streaming adapter. | No tracked Batch path, provider-enforced arbitrary JSON Schema, explicit cache control, Responses session persistence, or streaming adapter. |
...
| `GoogleGeminiAdapter` | n/a — the Google Gemini API adapter does not implement Oracle's tracked submit/retrieve contract                                                                                                                                       | n/a                                                                               |
| `DeepSeekAdapter`     | n/a — no native tracked Batch API path                                                                                                                                                                                                   | n/a                                                                               |
| `QwenAdapter`         | n/a — Alibaba documents OpenAI-compatible Batch, but Oracle's only batch caller is strict-schema extraction and Qwen's adapter is loose JSON mode. Do not advertise support without a safe caller and live proof.                       | n/a                                                                               |
...
apps/web/app/api/teams/notifications/route.ts   (the "always-on listener", on Vercel)
  ...
  • UPSERT meeting_transcripts (status='available') — DISCOVERY ONLY, no ingest
  ↓
admin picks meetings to ingest at /admin/transcripts → ingestMeetings()
  ↓
apps/workers/src/trigger/teams-transcript-ingestion.ts   (only for chosen meetings)
  ...
  • INSERT a channel (per call) + channel_participants + messages
    (role='user', extraction_status='pending', created_at ≈ real meeting time)
  • flip the meeting_transcripts row to 'ingested'
  ↓
existing claim-extraction cron picks up the pending messages  →  candidate-before-claim
```

**Meeting picker (2026-06-24):** the Oracle does NOT auto-ingest. The webhook and the on-demand `teams-transcript-discovery-scan` only RECORD available meetings (metadata) into `meeting_transcripts`; an admin chooses which to ingest at `/admin/transcripts`. Picking triggers ingestion → messages land `pending` (auto-extract), anchored to the real meeting time so an ingested past meeting isn't mistaken for a live one. See DECISIONS.md `D-meeting-picker` (supersedes the earlier `D-transcript-approval-gate`; the `awaiting_approval` enum value + `raw_transcripts.approval_status` are now unused).
...
Subscription lifecycle is owned by `teams-subscription-manager.ts`: the `teams-subscription-renew` cron (`*/30`) and a webhook-lifecycle-triggered task both call the idempotent `ensureAllSubscriptions()`, which keeps BOTH transcript subscriptions alive (renew if <20 min left, else create; a benign "limit of 1" 403 on create is treated as already-exists → renew). The resource max-lifetime is ~1h, so a machine must keep re-upping it — no human re-authenticates anything. Reuses the single existing 30-min schedule for both resources; Oracle currently keeps one of Trigger.dev's 10 schedule slots in reserve.

**Hard constraints (derived from Microsoft Graph, not choices):**

- **Discovery is "listen going forward."** A subscription only notifies for transcripts produced _after_ it existed; meetings during a subscription gap aren't pushed. To surface already-completed _scheduled_ meetings in the picker, the on-demand `teams-transcript-discovery-scan` task enumerates organizers and lists `getAllTranscripts(meetingOrganizerUserId=...,startDateTime=...)` via `users/{id}/onlineMeetings/getAllTranscripts` (the same enumeration `diagnose-transcripts.ps1` probes), upserting metadata into `meeting_transcripts` (no ingestion). Ad-hoc "Meet Now" calls are NOT enumerable that way — they are discoverable only via their live subscription.
- **No Graph live transcript / no Graph live spoken awareness.** Graph exposes no API to read a meeting's live caption/transcript stream, and Teams does not pipe spoken words into the meeting text chat. The Graph path therefore ingests calls _after_ they end. Live spoken participation is a separate Recall.ai meeting-bot path, not a Graph capability.
...
- `employee_identities` is the only identity source in the current schema and production database. Rerun-safe raw migration `98_drop_deprecated_employee_identity_columns.sql` removed the old `employees.auth_user_id`, `employees.auth_provider`, and `employees.auth_provider_subject` fields in release `30eed14` on 2026-07-29. A second full migration run and protected post-drop login both passed. The verifier supports both the all-present pre-drop state and the all-absent post-drop state, and fails loudly on partial state. See `DECISIONS.md` D2.multi-identity.
...
3. Classifies the query via `buildRetrievalPlanFromQuery` (heuristic keyword → `topDomainHints`, `excludedDocumentClasses`, `searchScope`). The opt-in `settings.entity_aware_retrieval_enabled=true` path uses `buildRetrievalPlanWithModel`: it performs a row-bounded canonical-registry lookup across the whole query, requires a whole label/canonical/alias surface, and lets only the primary configured `general` model route narrow those resolved candidates. The model cannot add an entity or trigger a model-pool retry. A required entity type overrides a conflicting broad heuristic exclusion, while multiple required entities keep any-of semantics. Failure emits `entity_aware_retrieval_fallback` and returns the existing keyword-only plan; with the setting absent or false, no extra model call occurs. The plan passes the employee's `departments` array as `departmentHints`, a soft RRF score bonus (+0.002 per matching claim) rather than a filter. Runs hybrid pgvector + tsvector RRF via `searchWithRetrievalPlan` with metadata pre-filter. Also fetches recent N messages, employee profile, and top open gaps for this employee/department.
4. Builds prompt blocks with the spec Part 10 system prompt plus the deterministic retrieval bundle. Multi-turn `messages`, `temperature`, and provider cache hints are passed through `providerOptions`; Vercel AI SDK tool definitions and Qwen session handles are not used by the direct adapters.
...
2. **Images run a two-pass flow:** Pass 1 (`transcribeImageToText`) resolves the admin-selected vision primary (`default_vision_route`) plus `model_pool_vision`, tries that approved chain in order, and fails loudly if it is unset or exhausted. The provider-direct model returns a faithful text rendering — a structured text topology for diagrams (nodes `[Shape: "label"]`, edges `[A] --(cond)--> [B]`, swimlane headers), verbatim labels kept inside the nodes. That text is the substrate for Pass 2.
3. Chunks the text into persisted `document_chunks` with **structure-aware chunking** (`chunkTextStructured`): it cuts only at semantic seams — heading lines (`### …`, numbered `N. ###`) and paragraph breaks, then line breaks, with a hard char cut only for a single over-long line — so a flowchart arrow line or a sentence is never severed across chunks. Then embeds them. Existing uploaded rows keep whatever chunks they already have; re-upload or deliberately recreate chunks to apply new chunking behavior.
4. Runs claim extraction over one or more labeled chunk windows via the extraction route. Each window is bounded for model reliability (`MAX_DOCUMENT_TEXT_CHARS`=24k, or `MAX_IMAGE_TEXT_CHARS`=32k for image transcriptions so a connected diagram is reasoned over in one call), but the worker covers every persisted chunk; there is no silent "first N characters only" document cap. When an image transcription looks like a diagram (`looksLikeDiagramTranscription`), the prompt asks for handoff/branch `dependency`/`exception` claims rather than one `process_rule` per box — but note each claim's `exactQuote` must still be a verbatim substring of ONE chunk, which only works when the transcription puts each edge on a single line (see AGENTS.md §10 "Diagram / flowchart image ingestion"). The prompt includes `DOCUMENT CHUNK` blocks and their `document_chunks.id` values, so document-derived candidates use a chunk ID as `sourceMessageId`. The uploader `context` is injected into the extraction prompt (and the Pass-1 vision prompt); `domain_hints` are a non-binding prior — per-claim `domain_valid` stays authoritative.
...
2. Resolves the synthesis primary from `settings.default_synthesis_route` plus the approved `model_pool_synthesis` chain, then routes through `OracleAIClient.runObject`. The router tries candidates in order through their direct provider adapters and fails loudly if the primary is unset or the pool is exhausted.
...
- `/admin/claims` — claim review queue with lateral join to primary evidence, asserting employee, and `claim_top_domains` domain chips. Approve/Reject/Revise/Assign server actions (`_actions.ts`) are audited in `claim_review_events`. Revise creates a replacement claim, copies evidence/domain/entity metadata, marks the original `superseded`, and links `claim_metadata.superseded_by_claim_id`. Admins can also edit an already approved claim; this uses the same supersede-and-replace path and creates a replacement in `pending_review`.
- `/admin/claim-groups` — admin-managed reusable recipient lists for claim-review questions. Assigning a claim to several people or a group expands into one `claim_review_question` gap per active employee, so each recipient sees an individual assignment.
- `/claims` — non-admin direct-assignment queue. A user can review a claim only when it was directly assigned to them through an open/queued/asked `claim_review_question` gap; broad domain-review queues are disabled for now.
...
- **Lull-driven** — `apps/workers/src/trigger/lull-interjection.ts` (R11.2). Each new user message schedules a trailing-debounced Trigger.dev run for that channel after 60 seconds. A newer message moves the pending check forward; the worker also verifies the triggering message is still latest before doing model or gap work. For that channel it queries `secondsSinceLastUserMessage`, `minutesSinceLastOracleInterjection`, count of interventions in the last hour, recent user messages, and open non-`model_coverage` gaps whose target is null or a channel participant. Search-only gap embeddings are compared with the recent-message embedding after those filters. Similarity must meet `lull_gap_minimum_relevance`; otherwise nothing is drafted or posted. Gap vectors never serve as claim evidence. Call `decideLullInterjection` (pure, in `packages/oracle-engines/src/interjection.ts`). On `'ask'`: draft the natural-language question via the configured interview route, insert the assistant message into `messages`, record `oracle_interventions` with `trigger_type='lull_gap'` + `was_live_interjection=true` + `interjection_message_id` + `related_gap_id`, update the gap `status='asked'` + `askedInMessageId`. With no new messages, this path creates no Trigger.dev runs.
...
- **Asking China-team members to verify a claim.** This reuses the existing claim-review-question mechanism (`assignClaimQuestion` → `gap_type='claim_review_question'`, with review groups + individual targets). The China addition: `assignClaimQuestionImpl` translates the question per recipient — a recipient whose `locale` is `zh-CN` (direct or via a group containing them) gets the question in Chinese (`translateReviewQuestionToChinese`, via the `translation` route), everyone else gets the original English. So a review question is translated only for China-team recipients. There is no separate recertification worker.
...
| Settings / model config           | `apps/web/app/admin/settings/model-pool` — per-stage and auxiliary pool checkbox table backed by the persisted `model_capabilities` Postgres table. **Stage requirements:** `apps/web/lib/stage-requirements.ts` is the shared source of truth for the pool page and the settings picker. Interview requires tools + structured output + vision + context >100K. Extraction requires structured output + context >100K, intentionally **not** vision because uploaded images are transcribed by the auxiliary Image Vision model before extraction receives text. Synthesis requires context >400K + structured output + reasoning + output-cap support. Auxiliary passes such as `vision`, `workflow_read`, `model_merge`, `translation`, `transcript_summary`, and `general` are registered in `packages/ai/src/routes/auxiliary.ts`; `teams-transcript-summary` resolves the dedicated `transcript_summary` slot (`default_transcript_summary_route` + `model_pool_transcript_summary`, seeded to `deepseek/deepseek-v4-flash` with Qwen fallback) before using its documented compatibility fallback to `general` if the slot is not configured. **Model discovery:** each of the 5 provider APIs is called in parallel (`sources/anthropic.ts` → Anthropic `/v1/models`, `sources/openai.ts` → OpenAI `/v1/models`, `sources/google.ts` → Google Gemini `/v1beta/models`, `sources/deepseek.ts`, `sources/qwen.ts`). **OpenAI filtering:** the OpenAI source uses a blocklist of non-chat categories (audio, image, realtime, TTS, transcription, moderation, video, legacy completion) rather than an allowlist of chat prefixes — new GPT/o-series generations pass through automatically. **Post-enrichment quality filters (all 5 providers):** (1) models with no pricing AND no capability flags are dropped; (2) models priced ≥ $15.01/1M input tokens are dropped. Pricing and capability flags come from OpenRouter (`sources/openrouter.ts` → `openrouter.ai/api/v1/models`, joined by model id with dash→dot + date-stripping normalization). All sources are fetched in parallel; per-source failures are non-fatal and surfaced in `errors[]`. **Defense in depth:** the same filter is applied again in `/api/admin/model-catalog` GET (`passesQualityFilter`) so rows already in the DB from before the write-time filter shipped are never returned to the admin UI. Existing DB rows are preserved (deprecated models may still be referenced by pool selections) — they're just filtered at read time. `/api/admin/model-catalog`: GET reads the table (filtered), POST triggers a full refresh (also filtered); `model-catalog-refresh-nightly` in Trigger.dev runs the same refresh every night at `15 7 * * *`. `/api/admin/models?stage=<>`: returns the configured pool for pipeline stages and auxiliary ids with pools, or the full catalog when a future auxiliary registry entry omits `poolSettingKey`. Pipeline and auxiliary callers use `resolveRouteCandidates(db, slot)` to resolve the admin-selected primary, continue through that slot's approved ordered pool, and fail loudly when the primary is unset or every candidate fails. `resolveModelRoute(modelIdOrRouteId, role)` remains the concrete id-to-route helper inside that flow and accepts both catalog `routeId`s and `provider/model` strings. |
...
- **Legacy identity columns were removed by raw migration 98.** All owned runtime readers and production authentication use `employee_identities`; post-drop deployment and protected-login proof passed in release `30eed14`.
...
Open work is tracked by owner, not by this historical retrofit summary. Start with `HANDOFF.md`, then use the STATUS table in `MACRO_FIRST_IMPLEMENTATION_PLAN.md`, `plan_repo_reliability_and_release_gaps.md`, `plan_deferred_product_and_infrastructure_gaps.md`, or `plan_typesafe_jev_decision_layer.md`. The Jev plan is the only authority for the proposed separate typed-decision path; it does not authorize adding Jev to the generative provider adapters. `DECISIONS.md` D6 + D9 record why the Vercel AI SDK and OpenRouter were ruled out; D10 + D11 record the live-interjection switch and the lull-interjection history; D12 records the DeepSeek + Qwen addition and the chosen API surfaces.
...
`packages/ai/src/routes/` defines `OracleModelRoute` and the curated route catalog. Route entries do
not point at fallback targets. Pipeline and auxiliary callers use `resolveRouteCandidates` to
resolve the selected primary plus that slot's ordered DB-approved model pool, then fail loudly if
the primary is unset or every candidate fails.
...
| `provider_response_sessions` | Reserved provider-native conversation/session state keyed by `(provider, session_key)`. The current Qwen Chat Completions path does not write this table; explicit Qwen session caching is disabled pending measured proof.                                                                                                                                                                                 |
...
Model reruns on this page are queued through `run_status` fields in `claim_extraction_ab_tests` and processed by the Trigger.dev `extraction-ab-eval` task. Do not move Gemini/Qwen calls back into the page server action; several simultaneous rows can exceed Vercel request time and leave the UI stuck on "Running models". Trigger.dev is currently at its schedule limit, so this path uses immediate task dispatch rather than adding a cron sweep.
...
Knowledge domains are associated with departments for possible claim review through `knowledge_domain_review_departments`. This is an authorization/routing map, not a retrieval signal: retrieval still uses `claim_top_domains`, entity tags, metadata, and employee department hints. The domain-review UI is currently disabled, and non-admin claim review is direct-assignment only through `claim_review_question` gaps. If domain review is restored later, the `/claims` page and `canReviewClaim()` action boundary must be updated together.
...
- _Same candidate, re-read inside the lock, status no longer `validated`_ → `reject(already_promoted)` (if another worker promoted _this_ candidate) or `reject(not_validated)` (if a sensitivity gate fired between reads).
- _Different candidate, same canonicalized hash_ → `append_to_existing_claim`. The current candidate is still `validated`; a parallel extraction of the same operational fact already committed a claim with the same hash. Our validated evidence is appended to their claim and our candidate is marked `duplicate`.
...
| `QwenAdapter`         | `openai` (custom baseURL to DashScope OpenAI-compat) | Provider-managed implicit prefix caching only; JSON-object mode + Zod validation for `generateObject`, with thinking forcibly disabled on structured calls; reported `prompt_tokens_details.cached_tokens` / `cache_creation_input_tokens` are normalized for observability but Oracle does not advertise or request explicit cache control |
...
The macro-first layer starts with a shape-aware source read. During document ingestion, after parsing/chunking/embedding and before claim extraction, `document-ingestion` awaits `generateSourceWorkflowMap()` (also exposed as the `source-workflow-read` Trigger task) when `map_directed_extraction_enabled=true` (the default). Stage 2 first runs a strict segmentation pass over all persisted `document_chunks` in order and classifies coherent material as `process`, `responsibilities`, `reference`, `ruleset`, `conversation`, or `narrative`. Every chunk must be covered; a genuinely composite chunk may belong to multiple focused segments because the production 4,000-character chunks can contain more than one shape. The deterministic validator rejects unknown IDs, repairs omissions into visible narrative fallback segments, splits non-contiguous model groupings, records per-shape assignment counts, and performs one bounded model repair retry when source-integrity repairs were required. The schema-validated model classification remains the dominant document shape because overlap makes a count-derived dominant shape misleading.
...
R2's first deeper responsibility-reader redesign is recorded in
`plan_r2_deeper_responsibility_architecture.md`. It separated exact-quote inventory acceptance from
field completeness, quarantined incomplete records, used one bounded combined repair call, and
expanded explicit destinations. Its production gate scored 12/30; the later approved GPT-4.1
bake-off averaged 11.5/30, so no second bake-off is allowed. The current successor design is
`plan_r2_source_span_inventory_reader.md`: exact source spans become the duty inventory, clear list
duties complete deterministically, and token-budgeted model calls handle only residual ambiguity.
Implementation remains open pending independent approval; no production gate is authorized early.
...
bounded raw-output hash/artifact are retained. A covered same-document cross-segment citation is
accepted and counted; a rate above 20% degrades the map.

R0 makes map rejection telemetry actionable. Each retained diagnostic includes the failing element and chunk, a bounded quote excerpt, failed check, selected policy, any alternate policy that would pass, validation method, cross-segment status, and root-versus-cascade lineage. The validator also retains a SHA-256 plus a bounded 64,000-character raw reader-output artifact so a future failure can be replayed without storing an unbounded response. Root quote/source failures are counted separately from edges and paths that disappear only because an endpoint failed. The source hash includes the reader pipeline version, so a validated map from an older reader is not incorrectly reused after a reader upgrade. A failed reader writes a failed map/job-run and marks `documents.macro_health='map_failed'`; by default (`require_workflow_map_for_ingestion=false`) ingestion continues through blind extraction with a degraded processing note. If that setting is true, reader failure still fails the document. Degraded maps remain usable but surface as degraded health.

The validated map is guidance only. It is rendered into every document claim-extraction window as `SOURCE WORKFLOW MAP (GUIDANCE ONLY - NEVER QUOTE THIS BLOCK)`. Extraction may set `mapElementRef` on candidates/claims to link a quote-backed atomic claim to a map element or relation, but `claim_evidence` must still quote one persisted `document_chunk`, and the deterministic quote validator remains the trust boundary. Before evidence validation or promotion, `mapElementRef` must resolve to the active map and its target chunk must be present in the current extraction window; well-formed fabricated, stale-map, and out-of-window references fail `map_element_ref_membership`. Map-referenced document candidates dedup by `(documentId, mapElementRef)` so a second valid phrasing of the same map element is marked duplicate instead of becoming another claim. Candidates without `mapElementRef` keep the existing summary/domain/quote/source hash.

After extraction, a DB-free shape registry deterministically reconciles each shape's primary map elements/relations to surviving claim refs. Missing primary refs upsert stable `gaps` rows with `gap_type='model_coverage'`; repeated reconciliation reuses the same UUID, covered nonterminal findings become stale, and resolved findings remain terminal. These rows are administrative model-quality findings, never employee questions: employee retrieval, the normal admin gap list, and lull-interjection queries explicitly exclude them. The writer stores stable source/map provenance in `gaps.source_context`. An admin may create one active reviewed conversion draft with a rewritten question, reason, and active recipients. Sending atomically creates ordinary `coverage_question` gaps and an append-only audit event; cancelling the draft creates no employee rows and exposes the form for a new draft. Source reads are also bounded before dispatch by configurable call, estimated-token, estimated-cost, repair, and concurrency limits; an over-budget source fails loudly and persists its budget snapshot in map/job diagnostics.

Legacy provisional outline/source-group tables remain for historical rows, but the writer workers and document lens fan-out subsystem were deleted in Stage 3. Document ingestion no longer fires `source-outline`, the old outline-injection flag was removed, and the UI no longer dispatches relationship/coverage writer tasks.

Durable macro relationships live in `macro_relationships` with support claim links in `macro_relationship_claims` and source lineage in `macro_relationship_sources`. The old LLM macro-relationship writer is gone; `/admin/macro` still lets admins approve, reject, manually author, drop stuck support, and run staleness sweeps. Historical `source_coverage_findings` remain administrative history and can be dismissed; they are not the current map-coverage conversion source.

### Cross-shape durable business model

R1 adds an unused-by-default shared spine beside the legacy process-only model. `business_objects`
and immutable `business_object_versions` own identity and versioning.
`business_elements`, `business_relations`, evidence links, system links, paths, and top-domain
links are shared across shapes. Six one-to-one detail tables hold fields that belong only to
process, responsibility, rule, reference, conversation, or limited narrative-macro elements.
The code-owned `BUSINESS_MODEL_SHAPE_REGISTRY` is the contract for allowed kinds, required fields,
reader and extraction instructions, persistence/render adapters, merge guidance, and coverage.

`business_model_changes` and `recommendations` accept either the new object identity or their
legacy process identity, never both. A create proposal has no object ID and locks the normalized
`object_kind + proposed_slug` namespace stored in dedicated columns. Existing-object work locks
the generic object ID. Legacy process columns and tables remain through R9. No legacy content is
copied in R1. New tables have RLS with no browser policy, and merge, apply, and serving flags all
default off.

R2 adds responsibility shadow proposals without creating business objects or versions. The merge
path requires current validated map-linked claims, validates typed fields and references, resolves
owner/department/system candidates while retaining honest raw names, hashes the immutable merge
input for same-map idempotency, and blocks namespace collisions from becoming creates. It resolves
the configured `model_merge` route and ordered fallback pool, calls a strict schema through
`OracleAIClient`, and writes context-pack, model-run, usage, and attempt provenance. Deterministic
shortlisting combines exact namespace with compatible semantic, owner, and top-domain signals;
current version IDs and durable element keys gate confirm/refine operations. Only evidenced
candidates enter merge, while sub-10% evidence omissions remain explicit. Same-input persistence
uses a transaction advisory lock. Document ingestion dispatches only after responsibility coverage
reaches 90% and the merge flag is on. Shadow
operations and evidence quotes render read-only in `/admin/business-model`. Every R2 proposal stores
`applyEligible=false`; the transaction service also enforces `business_model_apply_enabled` and
rejects shadow rows, so structural application remains unavailable before R6.

Macro relationships cite claim IDs rather than raw outline interpretation. Brain and chat read approved macro relationships only through a shared helper that verifies every support claim is currently `approved` at read time instead of trusting a denormalized relationship status. If a support claim leaves approved status through normal review actions, the application watcher marks affected approved relationships `stale_support`; the triggerable `macro-relationship-staleness-sweep` task is dispatched from the existing four-hour contradiction watcher sweep to catch drift from scripts or future code paths without consuming another Trigger schedule slot.
...
**Model routing + health (2026-07-07 / macro-first Stage 3).** The source workflow reader resolves the dedicated `workflow_read` slot (`default_workflow_read_route` + `model_pool_workflow_read`) because it is a long-context, flat structured-output pass. The `macro` slot remains reserved for later inferential macro passes. Macro health is still the document-level surface (`not_applicable | pending | complete | map_failed | map_degraded | merge_pending_review | degraded | failed`, `apps/workers/src/lib/macro-health.ts`) rendered in Admin → Documents.
...
- `approveTaxonomyProposal(id, reviewNote?)` is transactional. It applies `create_top_domain` inline. Other supported proposal types remain approved and visibly waiting until an admin clicks **Apply approved change**. That action dispatches the existing `taxonomy-reclassification` task for the exact proposal and records its Trigger run ID. The proposals page derives queued, applying, applied, skipped, and manual-intervention states from the append-only change log. Applied and skipped rows are terminal, so worker retries cannot repeat their changes.
