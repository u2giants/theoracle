...
| G7. Measure G6 in production | DONE 2026-08-27 - **REGRESSION, REVERTED** | Worker `20260827.3`, run `run_06g47pk9u7sceoth3nrvj48501`, map `eadb118c-0635-4ff5-a5fe-26f18865c1ae`: **13/30** with **seven preserved rows lost** (1, 2, 3, 4, 12, 18, 28) and only 46 stored records. All 192 completion outcomes were `provider_failed` with `omitted seeds`, including the exhaustive batch where no seed carried feedback, so the prompt is the cause. Feature reverted in `fa075cc`; worker `20260827.4` restores the `20260827.2` behaviour. All 16 gates pass on the reverted tree. |
| G8. Restore the served map | DONE 2026-08-27 | Albert named the map and the action. `eadb118c` demoted to `superseded`, `224ca68d-82c8-4954-ac65-59b02db00546` restored as the active `degraded` map, in one guarded transaction with preconditions checked and the single-active-map invariant proven before commit. Verified after the write: active map scores 23/30 with 95 records and all 19 prior rows preserved. No map deleted. |
| G9. Live prompt-contract check | DONE 2026-08-27, PASSING | `verify:r2-completion-contract-live` sends 8 real seeds through the exact production call path and asserts one record per requested seed. Baseline on the current prompt: **8 requested, 8 returned, zero failures**. The probe exits explicitly because `getDirectDb()` holds an open pool. The Oracle key's previously missing `sk-proj-` prefix was verified repaired in 1Password on 2026-09-08. Run the probe before any production gate carrying a completion-request change. |
| G10. Owner authorization of one bounded correction cycle | DONE 2026-09-08 | Albert said “fund one tightly bounded R2 correction cycle.” This authorizes one code correction, the existing gates, one live contract probe, one worker deployment, and exactly one production map with `maxAttempts: 1`; it does not authorize a retry, scorer/validator/threshold changes, broader production writes, or secret rotation. GitHub issue #4 tracks closure. |
| G11. Put validator feedback in each rejected seed without changing the global prompt | DONE 2026-09-08 | Commit `c6d4dd1` plus its handoff commit were reviewed, passed all pre-deploy gates on `al8960ofc`, merged through PR #5 as `b4933b8`, and exact-head main CI run `34260560354` passed. The global completion system prompt remains unchanged at v1. |
| G12. Licensed gates, deploy once, run once, score and stop | STOPPED AFTER THE ONE DEPLOY ATTEMPT 2026-09-08 | All listed pre-deploy checks passed: pinned inventory 28/30 with rows 16/26 unsupported; production replay 19 baseline / 21 corrected with no regressions; live feedback-path contract 8 requested / 8 returned / zero failures; frozen route, budgets, safety flags, document, and prior 23/30 map matched. The single authorized Trigger.dev deploy attempt built the code, then failed before promotion with `The "data" argument must be of type string ... Received undefined`. The written gate says a deploy failure stops without a trigger. Therefore no worker was promoted, no production task or map was created, no score exists for this cycle, and no retry is authorized. Issue #4 remains open. |
| G13. Diagnose the deploy failure and make one new deploy attempt | DONE 2026-09-08 | Albert authorized the new bounded deploy attempt after G12 stopped. The failing connector had run a `4.4.6 -> 4.5.16` version transition while the repository pins Trigger.dev `4.5.15`. The exact pinned `4.5.15` dry run passed; the one newly authorized production deploy then succeeded as deployment `i7fdgszi`, worker `20260908.1`, from merged commit `b4933b8`. The worker was independently confirmed current with SDK `4.5.15` and all 25 tasks. |
| G14. Run exactly one production map on worker `20260908.1` | DONE 2026-09-08 - **GATE FAILED HONESTLY** | Albert explicitly authorized exactly one production `source-workflow-read` run with `maxAttempts: 1` and no retry. Run `run_06g856791rotomf2a1hu7vrd01` completed once and created active degraded map `905e0626-d9a2-4014-a6a3-97e572214896`. The frozen scorer returned **23/30**, below the 27/30 threshold, with all 19 prior rows preserved, recovered rows 17/19/20/29, and negative controls 16/24/26 unmatched. This equals rather than regresses from the prior 23/30 map, so the authorized regression restore did not apply. All 16 post-score gates and `git diff --check` passed. No retry, tuning, migration, deletion, or rollback occurred. Issue #4 remains open. |
| G15. Correct the four remaining supported misses locally | DONE 2026-09-08; PRODUCTION NOT AUTHORIZED | Read-only diagnosis of map `905e0626-...` found rows 5/14/15/23 all had faithful records whose action/object wording failed the frozen matcher. The final-record corrector now canonicalizes only fidelity-passing completion records back to their exact single-duty source-bound action/object, then requires the unchanged fidelity validator to pass. The correction contract is 17/17; all 14 non-credentialed gates, pinned 28/30 inventory, and 8/8 live contract probe pass. Production replay improves 21 to 22, preserves all 19 baseline rows, adds no record regression, and keeps frozen controls unchanged. No deploy or production map run occurred. |
