...
| G9. Live prompt-contract check | BUILT 2026-08-27, NOT YET EXECUTED | `verify:r2-completion-contract-live` sends 8 real seeds through the exact production call path and asserts one record per requested seed. Typechecked, wired, and proven as far as the model dispatch: DB connects, route resolves to `openai/gpt-4.1`, all 8 seeds pack. It cannot complete on this machine because `OPENAI_API_KEY` is not in the `vibe_coding` vault. Storing it is the one remaining step. |
