...
| G9. Live prompt-contract check | DONE 2026-08-27, PASSING | `verify:r2-completion-contract-live` sends 8 real seeds through the exact production call path and asserts one record per requested seed. Baseline on the current prompt: **8 requested, 8 returned, zero failures**. Two fixes were needed: the vault's OpenAI key is valid but stored without its `sk-proj-` prefix (item notes updated), and the probe had to exit explicitly because `getDirectDb()` holds an open pool. Run it before any production gate carrying a prompt change. |
