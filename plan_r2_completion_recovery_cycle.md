...
| G9. Live prompt-contract check | BUILT 2026-08-27, NOT YET EXECUTED | `verify:r2-completion-contract-live` sends 8 real seeds through the exact production call path and asserts one record per requested seed. Typechecked, wired, and proven as far as the model dispatch: DB connects, route resolves to `openai/gpt-4.1`, all 8 seeds pack. It cannot complete because all three OpenAI keys in `vibe_coding` return HTTP 401 (verified 2026-08-27 against `/v1/models`); the item's notes now record that. A working key exists in the Trigger.dev prod environment. Getting one valid key into the vault is the one remaining step. |
