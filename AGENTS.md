...
| Macro-first redesign / shape-aware reader / business model layer / workflow maps / source outlines / cross-claim process relationships | `SHAPE_AWARE_READER_DESIGN.md` for the approved reader stages, `MACRO_FIRST_REDESIGN.md` for the broader forward plan, `MODEL_BAKEOFF_SPEC.md` for model selection, `evals/{shape-aware-stage2,macro-first-battery}.md` for stage-gate results, `fix_enhancement.md` §1–§6 as diagnosis record + ground truth only, `AGENTS.md`, `docs/architecture.md`, `DECISIONS.md` when changing provenance or routing rules | `docs/macro-understanding-implementation-plan.md` (pre-redesign plan, historical), deployment docs unless worker schedules/env/deploy behavior changes |
