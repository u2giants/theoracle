...
| Coverage findings table | `source_coverage_findings` | `packages/db/src/schema.ts`, migration `79_macro_understanding.sql` | Historical coverage audit findings over a source outline; no new LLM coverage writer after Stage 3. Admins can still dismiss findings or convert them into gaps. |
...
| implementation + live gate complete; release proof open | Chat attachment/cache safety keeps every attachment in the provider-neutral canonical message. Vertex removes only the exact hash-matched cached PDF from its own live request after cache creation; other PDFs/images remain inline, and a fallback provider receives the complete canonical message. Download, unsupported-file, unsafe-size, and required-cache failures stop clearly. Oversized requests are Vertex-only only when the cached PDF makes the remaining live payload safe; a failed required GCS/cache preparation cannot fall through to the oversized inline call. The credentialed live gate passed on 2026-07-28: after a forced pre-network Vertex failure, Anthropic read both generated PDFs. | Commit, push, and record fresh green CI before closing GAP-5. Network-free guards: `verify:chat-attachment-safety`, `verify:vertex-file-cache`, and `file-part-translation.ts`. |
