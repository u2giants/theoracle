...
| local complete; release proof open | GAP-12 lull questions scan all eligible gaps in bounded keyset pages, require topical relevance to the recent user-message window, fail closed without real embeddings, and recheck typing under the advisory lock. Search-only gap vectors do not change claim evidence. | Review and apply raw migration 101 through the journal, pass CI, deploy the Trigger worker, then prove one related ask and one unrelated no-post. Guards: `verify:lull-topical`, `verify:gap-topical-schema`, `verify:vercel-contract`. |
