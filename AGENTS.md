...
| open | **Synthesis run once, not yet fully exercised.** 2026-06-26: prod has **215 approved** claims (123 `pending_review`) and `brain_section_versions` = 1 (synthesis has executed at least once). A fuller run against the now-larger approved corpus is the remaining demo. | Trigger `brain-synthesis` and confirm the Brain narrative reflects the current approved-claim corpus. |
