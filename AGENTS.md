...
|---|---|---|
| open | `apps/web/app/admin/taxonomy/_actions.ts` records queued work, and the existing `taxonomy-reclassification` worker can apply several proposal types, but approval never dispatches it. | Follow `plan_deferred_product_and_infrastructure_gaps.md` GAP-1. Finish and expose the existing path; do not add a second worker. |
...
| local complete; live gate open | China side-by-side translation review, append-only review history, stale/pending/rejected serving protection, and network-free CI/Vercel guards are implemented locally. Translation remains opt-in. | Run `pnpm --filter @oracle/ai verify:chinese-retrieval-live` with the real embedding credential, then record release proof before closing GAP-4. |
...
| local complete; release proof open | **GAP-13 oversized conversation windowing.** Sync and Batch extraction share model-bounded, message-only windows with configurable active overlap. Every repeated message keeps its evidence ID and timestamp; carry-in is non-quotable; existing candidate identity deduplicates overlap; one unfit message fails without truncation. | Review migration 102, run CI, deploy the worker, and prove an oversized conversation against the smallest configured extraction context. Guard: `verify:conversation-windowing`. |
