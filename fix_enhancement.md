...
> **⚠️ STATUS BANNER — this section is STALE as of 2026-07-04 (audit).** Read against the working tree, not this section. Items listed below or later as "still open / not yet implemented / deferred" that are in fact DONE in the current code: **Bug G** (coverage is now sequenced *after* macro inside `macro-relationship-extraction.ts`, not raced), the **admin `macro_health` UI** (rendered in Admin → Documents), **Bug D** (`statusForGeneratedMacroRelationship` now births generated relationships `pending_review`/admin-visible while read-time serving still gates on approved support), and the **`source_workflow_maps` workflow-map artifact** (schema + migration `86`, edge-level dedup via `candidate-hash.ts` `stableGraphKey`, deterministic macro derivation from graph paths). The macro primary shipped as `openai/gpt-4.1-mini` (Gemini rejects the nested schema), not `google/gemini-2.5-flash`. The before/after extraction *reorder* remains genuinely open. See `HANDOFF.md` (2026-07-04 audit entry) and `bug_d_ungating_plan.md`.

### Implemented this session

1. **Dedicated `macro` model slot (fixes the ERR-001 proximate cause).** The macro layer no longer borrows the `general` utility slot. New auxiliary slot `macro` with its own admin picker **`default_macro_route`** (Admin -> Settings -> "Macro understanding model"), a `requiredCapability: 'structuredOutputs'` filter, and a comprehensive **"Copy job brief"** (spells out that STRICT json-schema — not loose `json_object` — is a hard requirement, that it runs before *and* after atomic extraction, and what capabilities help/hurt). The live/default primary is now `openai/gpt-4.1-mini`; Gemini 2.5 flash/pro rejected the nested macro relationship and coverage schemas as too complex and remain fallback/context only.
   ...
   - **Live status:** migrations 83/84/85 were applied and worker `20260703.4` verified macro/coverage success. Migration 86 (`source_workflow_maps`) was applied and worker `20260704.2` deployed the structural workflow-map follow-up; real swimlane rerun verification remains.
   - **Hardening status:** fallback pool exists, and a one-shot schema-repair pass is now wired for macro relationship and coverage validation failures.

2. **`AGENT_ERROR_LOG.md`** — a designated, in-repo, agent-readable failure log, referenced from `HANDOFF.md`. Seeded with ERR-001 (Qwen schema hard-fail) and ERR-002 (cross-source SQL), then updated with ERR-003/ERR-004 for followup sequencing and workflow maps. Rule: runtime failures must be captured here in a form specific enough to change code from — because a failure that lives only in Trigger.dev/`model_run_attempts` and nobody reads *is* a silent failure.
...
7. **Workflow map + graph-aware extraction (structural fix).** `source_workflow_maps` persists nodes/edges/paths/swimlanes; document ingestion performs a before-extraction outline/workflow-map read; extraction candidates store `workflowTrace` and can dedupe by stable workflow edge key; macro extraction derives deterministic path relationships from traced edges; coverage audit emits missing-edge findings.
8. **Bug D + Bug G resolved in source.** Generated macro relationships with pending support now enter `pending_review` for admin visibility while Brain/chat still require approved support at read time. Coverage is sequenced after macro, not raced beside it.

Still not done: real swimlane/Pop-style document rerun verification and broader historical backfill/eval beyond the focused Pop-style fixture.

### The "macro before AND after atomic extraction" requirement — current state
...
**Current source state:** `document-ingestion.ts` now generates/reuses the source outline immediately after chunk insertion and before broad atomic extraction, with followup orchestration deferred. The broad extraction then injects the fresh outline/workflow map as non-quotable guidance. The normal post-ingestion source-outline trigger still runs afterward; when it finds the matching outline, it orchestrates lens fan-out, macro relationship extraction, and coverage audit. This gives the intended before read plus after relationship/coverage pass without making outline prose valid evidence.

**Still needs prod verification:** rerun the swimlane/Pop-style document and verify `source_workflow_maps`, `workflowTrace`, deterministic macro relationships, missing-edge findings, and `pending_review` macro visibility. Migration 86 is applied and worker `20260704.2` is deployed.
