...
export default async function AdminBusinessModelPage() {
  ...
  const [processCount, proposalCount, mapCount, processes, proposals, maps] =
    await Promise.all([
    ...
    <div className="space-y-6">
      <header className="space-y-1">
        ...
        <p className="text-sm text-muted-foreground">
          Read-only Stage 1 surface for source workflow maps, model-change proposals,
          and durable business processes.
        </p>
      </header>
      ...
      <div className="grid gap-3 md:grid-cols-3">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Source workflow maps</CardDescription>
          </CardHeader>
        </Card>
        <Card>
          <CardHeader>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Approved versions will become the primary answering context.</CardDescription>
        </CardHeader>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Source workflow maps</CardTitle>
          <CardDescription>Stage 2 will write validated per-source maps here.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          {maps.length === 0 ? (
            <EmptyState label="source workflow maps" />
          ) : (
            maps.map((map) => (
