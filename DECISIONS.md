...
- **Coverage rule**: Primary map-to-claim coverage is a deterministic shape-registry reconciliation, not an LLM score. Omissions upsert stable `model_coverage` gap IDs. Those rows are administrative model-quality findings and every employee-facing gap consumer excludes them; converting one into a human question requires a separate future audited action.
