...
- **Deprecated columns removed after proof**: the read-only production audit found all three old `employees.auth_*` columns empty with no external dependencies, and a rollback-compatible release authenticated successfully. Forward-only raw migration `98_drop_deprecated_employee_identity_columns.sql` removes them; production execution and post-drop release/authentication proof remain pending.
