...
- **Decision**: The `OracleProviderAdapter` interface gains two **optional** methods: `submitBatch(args)` and `retrieveBatch(args)`. Adapters that don't implement them are unchanged. Round 1 wires Anthropic-style Batch APIs on **OpenAI** (`client.batches.create` + JSONL via `client.files`) and **Vertex Gemini** (`client.batches.create` with GCS-backed JSONL I/O). DeepSeek and Qwen are deferred — DeepSeek has no public batch API; Qwen's batch surface is non-OpenAI-compatible and would require a native DashScope SDK swap, which D12 already deferred.
...
- **Round 1 scope (this session)**: foundation only — schema, adapter contract, OpenAI + Vertex adapter implementations, typecheck green. The two-phase worker (claim-extraction submit task + drain cron) and the `extraction_dispatch_mode` settings flag are deferred to a follow-up session because they involve a 400+ line refactor of the existing `claim-extraction.ts` `processSegment` flow to share per-segment processing between sync and batch paths. See `HANDOFF.md` for the precise next-action.
