    ...
    return await db.transaction(async (tx) => {
      ...
      if (proposal.processId && proposal.baseVersionId) {
        const [process] = await tx
          ...
          .where(eq(businessProcesses.id, proposal.processId))
          .limit(1);

        const actualVersionId = process?.currentVersionId ?? null;
        if (actualVersionId !== proposal.baseVersionId) {
          ...
          const [updated] = await tx
            ...
            .where(eq(businessModelChanges.id, proposal.id))
            .returning();

          await tx.insert(businessModelChangeEvents).values({
            businessModelChangeId: proposal.id,
            action: 'needs_rebase',
      ...
      const value = await apply(tx, proposal);
      return { status: 'applied', proposal, value };
    });
  } catch (err) {
    await db
      .update(businessModelChanges)
