...
VALUES
  ...
  ('default_workflow_read_route', '"openai/gpt-4.1"'::jsonb, 'Model route for source workflow read. OpenAI first: the 2026-07-07 Stage 2 gate found claude-sonnet-5 rejects the adapter temperature parameter and Gemini 2.5 Pro rejects the schema as too complex.', now()),
  ('model_pool_workflow_read', '["openai/gpt-4.1","anthropic/claude-sonnet-5","google/gemini-2.5-pro"]'::jsonb, 'Ordered fallback chain for source workflow read. OpenAI first based on the live Stage 2 gate.', now()),
